...
- **Exit settlement:** the first valid exit scan ends the session. Available deposit is applied to overtime; any remainder becomes an Outstanding Charge that must be collected or explicitly waived before financial settlement is complete. The Exit Scanner may record only this deterministic package-computed settlement; it cannot change the amount, apply a general refund, use a Price Override, or waive a charge. Exceptions go through an authorized Cashier/Owner correction flow with an audit reason.
