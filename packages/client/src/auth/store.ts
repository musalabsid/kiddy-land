...
export const useAuthStore = create<AuthState>((set) => ({
  ...
  clearSession: () =>
    set((state) => ({ session: undefined, pairedDevice: state.pairedDevice })),
  hydrated: false,
...
export function readStoredDevice(): SessionInfo["device"] | undefined {
  ...
  try {
    return JSON.parse(
      window.localStorage.getItem(DEVICE_STORAGE_KEY) ?? "null",
    ) as SessionInfo["device"] | undefined;
  } catch {
    return undefined;
  }
}
