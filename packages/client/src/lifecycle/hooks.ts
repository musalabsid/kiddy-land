...
export function useTicketScan(kind: "entry" | "exit") {
  ...
  return useMutation({
    mutationFn: (code: string) => {
      ...
      return client.post<ScanResult>(`/tickets/scan/${kind}`, { code });
    },
  });
}
export function useTicketRecovery() {
  ...
  const connection = useConnectionStore();
  return useMutation({
    mutationFn: ({ code, childId }: { code: string; childId: string }) => {
      if (!canMutate(connection.state, connection.synchronized))
        throw new Error("Connection is not synchronized");
      return client.post<{ ticketId: string; code: string; qrToken: string }>(
        "/tickets/recover",
        { code, childId },
      );
    },
  ...
  return useMutation({
    mutationFn: ({
      ...
      return client.post<{
        ...
        collectedAt: number;
        session: PlaySession;
      }>(`/tickets/${ticketId}/collect-charge`, { amount, paymentMethod });
    },
  });
}
  ...
  return useMutation({
    mutationFn: (ticketId: string) => {
      ...
      return client.post<{
        ...
        amount: number;
        refundedAt: number;
      }>(`/tickets/${ticketId}/refund-deposit`, {});
    },
  });
}
