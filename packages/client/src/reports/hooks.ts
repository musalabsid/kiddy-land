...
export function useReport<T>(kind: "financial" | "playground" | "inventory" | "membership", filters: ReportFilters, opts?: { enabled?: boolean }) { const client = useClient(); const query = qs(filters); return useQuery({ queryKey: [...clientQueryKeys.reports, kind, query], enabled: opts?.enabled ?? true, queryFn: () => client.get<T>(`/reports/${kind}${query ? `?${query}` : ""}`) }); }
