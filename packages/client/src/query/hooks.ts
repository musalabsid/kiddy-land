...
export function useSessionQuery() {
  ...
  return useQuery({
    queryKey: clientQueryKeys.session,
    queryFn: () => client.get("/auth/session"),
    enabled: Boolean(client.getToken()),
  });
}
...
export function useOwnerLoginMutation() {
  ...
  return useMutation({
    mutationFn: (password: string) =>
      new AuthService(client).ownerLogin(password),
    onSuccess: async (result) => {
      ...
      writeStoredDevice(current.device);
    },
  });
}
...
export function useBootstrapStatusQuery() {
  ...
  return useQuery({
    ...
    queryFn: () => new AuthService(client).bootstrapStatus(),
  });
}
...
export function useBootstrapMutation() {
  ...
  return useMutation({
    mutationFn: (password: string) =>
      new AuthService(client).bootstrap(password),
    onSuccess: (result) => {
      queueMicrotask(() => {
        client.setToken(result.session.token);
        const session = {
          token: result.session.token,
          deviceId: result.session.deviceId,
          device: result.device,
          user: { id: "owner", username: "owner", role: "Owner" as const },
        };
        setSession(session);
        writeStoredSession(session);
...
export function useInvitationMutation() {
  ...
  return useMutation({
    ...
    }) => new AuthService(client).createInvitation(origin, kind, staff),
  });
}
...
export function useDevicesQuery() {
  ...
  return useQuery({
    ...
    queryFn: () => new AuthService(client).listDevices(),
  });
}
...
export function useRevokeDeviceMutation() {
  ...
  return useMutation({
    mutationFn: (deviceId: string) =>
      new AuthService(client).revokeDevice(deviceId),
    onSuccess: () =>
      void queryClient.invalidateQueries({ queryKey: ["pairing", "devices"] }),
  });
}
...
export function useDeleteDeviceMutation() {
  ...
  return useMutation({
    mutationFn: (deviceId: string) =>
      new AuthService(client).deleteDevice(deviceId),
    onSuccess: () =>
      void queryClient.invalidateQueries({ queryKey: ["pairing", "devices"] }),
  });
}
...
export function usePairingMutation() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      mode: DeviceMode;
      origin?: string;
    }) => new AuthService(client).pair(token, mode, origin),
    onSuccess: (result) => {
      ...
      queueMicrotask(() => {
        ...
        if (result.session) {
          client.setToken(result.session.token);
          const session = { ...result.session, device: result.device };
          setSession(session);
          writeStoredSession(session);
        }
        void queryClient.invalidateQueries({
...
export function useRestoreSession() {
  ...
  React.useEffect(() => {
    ...
    client
      .get<{ device: typeof stored.device; user?: typeof stored.user }>(
        "/auth/session",
      )
      .then((current) => setSession({ ...stored, ...current }))
      .catch(() => {
        client.setToken(undefined);
        clear();
        writeStoredSession(undefined);
...
export function useLoginMutation() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      username: string;
      password: string;
    }) => new AuthService(client).login(deviceId, username, password),
    onSuccess: async (result) => {
      if (!pairedDevice) return;
      client.setToken(result.token);
      const current = await client.get<{
        device: typeof pairedDevice;
        user?: SessionInfo["user"];
      }>("/auth/session");
      const session = {
        token: result.token,
      ...
      queueMicrotask(() => {
        setSession(session);
        writeStoredSession(session);
        queryClient.invalidateQueries({ queryKey: clientQueryKeys.session });
      });
    },
  });
}
...
export function useChangePasswordMutation() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      newPassword: string;
    }) => new AuthService(client).changePassword(currentPassword, newPassword),
  });
}
