...
export function createNotificationInbox() {
  ...
  return {
    receive(event: ServerEvent) { if (event.type === "notification") { alerts = [...alerts, event]; for (const listener of listeners) listener(event); } },
    list() { return alerts; },
    dismiss(id: string) { alerts = alerts.filter((event) => event.id !== id); for (const listener of listeners) listener({ type: "notification-dismissed", id }); },
