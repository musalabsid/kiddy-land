...
describe("shared client", () => {
  test("adds auth and parses successful JSON responses", async () => {
    ...
    try {
      const client = new ApiClient("http://local.test", "secret");
      expect(await client.get<{ status: string }>("/ready")).toEqual({
        status: "ready",
      });
    } finally {
  ...
  test("normalizes API failures and blocks unsynchronized writes", async () => {
    ...
    try {
      await expect(
        new ApiClient("http://local.test").get("/private"),
      ).rejects.toBeInstanceOf(ClientError);
    } finally {
      globalThis.fetch = previous;
    }
    expect(canMutate("connected", false)).toBe(false);
    expect(canMutate("connected", true)).toBe(true);
  });
});
