    ...
    <Card>
      ...
      <CardContent className="grid gap-3">
        <p className="text-xs text-muted-foreground">
          {t("sale.todayCount").replace("{count}", String(sales.length))} ·{" "}
          {operatingDate}
        </p>
        {sales.length === 0 ? (
          ...
          sales.map((sale) => {
            ...
            const memberLabel = sale.lines.find((l) => l.memberId)?.memberId
              ? `${t("sale.member")}`
              : t("sale.guest");
            return (
                ...
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="min-w-0">
                    <p className="font-mono text-xs text-muted-foreground">
                      {sale.receipt.number} · {time} ·{" "}
                      {sale.status === "void" ? "void" : sale.paymentMethod}
                    </p>
                    <p className="truncate text-sm font-medium">
                      {formatIdr(sale.total, locale)} · {sale.tickets.length}{" "}
                      {t("sale.ticket")}
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    ...
                    {sale.tickets.length > 0 ? (
                      <Button
                        ...
                        variant="outline"
                        onClick={() =>
                          void artifact(sale.id, "tickets", "open")
                        }
                      >
                        <Printer
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => void artifact(sale.id, "receipt", "open")}
                    >
                      <ExternalLink
                        data-icon="inline-start"
                        className="size-3.5"
                      ...
                      >
                        <span className="truncate">
                          ...
                          {sale.lines.find(
                            (l) =>
                              l.kind === "ticket" &&
                              (l as unknown as { ticketId: string })
                                .ticketId === ticket.id &&
                              (l as unknown as { memberId?: string }).memberId,
                          ) ? (
                        ...
                        <div className="grid gap-1">
                          <Button
                            ...
                            onClick={() => {
                              if (qrUrls[ticket.id]) {
                                URL.revokeObjectURL(qrUrls[ticket.id]!);
                                setQrUrls((prev) => {
                                  const n = { ...prev };
                                  delete n[ticket.id];
                                  return n;
                          ...
                          >
                            {qrUrls[ticket.id]
                              ? (t("sale.hideQr") ?? "Hide QR")
                              : t("sale.showQr")}
                          </Button>
                          {qrUrls[ticket.id] ? (
                            <img
                              src={qrUrls[ticket.id]}
                              alt="QR"
                              className="mt-1 size-32 rounded border bg-white p-1"
                            />
                          ) : null}
                        </div>
                      </div>
                    ...
                    <div className="flex flex-wrap gap-1.5 pt-1">
                      {sale.tickets.length > 0 ? (
                        <Button
                          ...
                          variant="outline"
                          onClick={() =>
                            void artifact(sale.id, "tickets", "download")
                          }
                        >
                          <Download
                            data-icon="inline-start"
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={() =>
                          void artifact(sale.id, "receipt", "download")
                        }
                      >
                        <Download
                          data-icon="inline-start"
