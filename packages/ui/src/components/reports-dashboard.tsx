...
function fmtIDR(n: number, locale: string) {
...
export function ReportsDashboard() {
  ...
  const [kind, setKind] = React.useState<
    "financial" | "playground" | "inventory" | "membership"
  >("financial");
  ...
  const {
    ...
    defaultValues: { from: today, to: today },
  });
  const filters = {
    from,
    to,
  ...
  const financial = useReport<{
    data: FinancialData["totals"] extends never
      ? any
      : { totals: FinancialData["totals"] };
  }>("financial", filters) as any;
    ...
    <div className="w-full max-w-6xl px-5 py-8 sm:px-8">
      <header className="mb-4 flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={() => submitDownload("csv")}>
            <Download data-icon="inline-start" />
            CSV
          </Button>
          <Button onClick={() => submitDownload("pdf")}>
            <Download data-icon="inline-start" />
            PDF
          </Button>
        </div>
      ...
      <section className="grid gap-4">
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription>{t("reports.exportsHint")}</CardDescription>
          </CardHeader>
        ...
        <div
          role="tablist"
          className="flex w-fit flex-wrap gap-1 rounded-lg bg-muted p-1"
        >
          {KINDS.map((k) => (
            <button
              key={k.id}
        ...
        {isLoading ? (
          ...
          <Alert variant="destructive">
            ...
            <AlertDescription>{t("reports.checkDates")}</AlertDescription>
          </Alert>
        ) : kind === "financial" ? (
          <FinancialView data={financial.data as any} locale={locale} />
        ) : kind === "playground" ? (
          <PlaygroundView data={report.data as any} />
        ) : kind === "inventory" ? (
          <InventoryView data={report.data as any} />
        ) : (
          <MembershipView data={report.data as any} />
        )}

        {report.data && (
          <p className="text-xs text-muted-foreground">
            {t("reports.generated")}{" "}
            {(report.data as any)?.generatedAt
              ? new Date((report.data as any).generatedAt).toLocaleString(
                  locale,
                )
              : ""}{" "}
            · {(report.data as any)?.timezone ?? ""} ·{" "}
            {(report.data as any)?.filters?.from} →{" "}
            {(report.data as any)?.filters?.to}
          </p>
        )}
      </section>
    </div>
    ...
    <div className="grid gap-4">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Stat
          ...
          value={fmtIDR(vals.grossRevenue, locale)}
          sub="tickets + products + overtime"
        />
        <Stat
          label={t("reports.netRevenue")}
          value={fmtIDR(vals.netRevenue, locale)}
          sub="gross − refunds + corrections"
          accent
        />
        <Stat
          label={t("reports.ticketRevenue")}
          value={fmtIDR(vals.ticketRevenue, locale)}
        />
        <Stat
          label={t("reports.productRevenue")}
          value={fmtIDR(vals.productRevenue, locale)}
        />
      </div>
      <Card>
        <CardHeader className="pb-3">
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2">
          <div className="grid gap-2 text-sm">
            ...
            <Row
              ...
              value={"-" + fmtIDR(vals.refunds, locale)}
            />
            <Row label={t("reports.voids")} value={String(vals.voids)} />
            <Row
              label={t("reports.correctionNet")}
              value={fmtIDR(vals.correctionNet, locale)}
            />
            <Row
              label={t("reports.priceOverrides")}
              value={String(vals.priceOverrides?.length ?? 0)}
            />
          </div>
          <div className="grid gap-2 text-sm">
            <Row
              label={t("reports.depositsReceived")}
              value={fmtIDR(
                vals.deposits?.received ?? vals.depositsReceived ?? 0,
                locale,
              )}
            />
            <Row
              label={t("reports.depositsApplied")}
              value={fmtIDR(vals.deposits?.applied ?? 0, locale)}
            />
            <Row
              label={t("reports.depositsRefunded")}
              value={fmtIDR(vals.deposits?.refunded ?? 0, locale)}
            />
            <Row
              label={t("reports.depositsHeld")}
              value={fmtIDR(vals.deposits?.held ?? 0, locale)}
            />
            <Row
              label={t("reports.forfeited")}
              value={fmtIDR(vals.deposits?.forfeited ?? 0, locale)}
            />
          </div>
        </CardContent>
      </Card>
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          ...
          <CardContent>
            {Object.keys(vals.paymentMethods || {}).length ? (
              <div className="grid gap-1 text-sm">
                {Object.entries(vals.paymentMethods).map(([k, v]) => (
                  <Row key={k} label={k} value={fmtIDR(v as number, locale)} />
                ))}
              </div>
            ) : (
        ...
        <Card>
          ...
          <CardContent>
            {Object.keys(vals.cashiers || {}).length ? (
              <div className="grid gap-1 text-sm">
                {Object.entries(vals.cashiers).map(([k, v]) => (
                  <Row
                    key={k}
                    label={k.slice(0, 8)}
                    value={fmtIDR(v as number, locale)}
                  />
                ))}
              </div>
            ) : (
              <p className="text-sm text-muted-foreground">
                {t("reports.noCashiers")}
              </p>
            )}
          </CardContent>
        </Card>
...
function PlaygroundView({ data }: { data: any }) {
  const { t } = useLocale();
  const d = data?.data;
  if (!d || typeof d.occupancy === "undefined")
    return (
      <Card>
        <CardContent className="p-8 text-center text-base text-muted-foreground">
          {t("reports.noPlayground")}
        </CardContent>
      </Card>
    );
  return (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <Stat
        label="Auto-closed"
        value={String(d.autoClosed ?? 0)}
        sub={d.overtimeMinutes ? d.overtimeMinutes + " overtime min" : ""}
      />
    </div>
  );
}
  ...
  return (
    <div className="grid gap-4">
      <div className="grid gap-3 sm:grid-cols-3">
        <Stat label="Products" value={String(d.products?.length ?? 0)} />
        <Stat
          label="Low stock"
          value={String(d.lowStock?.length ?? 0)}
          tone={d.lowStock?.length ? "destructive" : "default"}
        />
        <Stat label="Movements" value={String(d.movements?.length ?? 0)} />
      </div>
      {d.lowStock?.length ? (
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="grid gap-2 text-sm">
              {d.lowStock.slice(0, 8).map((p: any) => (
                <div
                  key={p.id}
      ...
      {d.products?.length ? (
        <Card>
          ...
          <CardContent>
            <div className="grid max-h-64 gap-1 overflow-auto text-sm">
...
function MembershipView({ data }: { data: any }) {
  ...
  if (!d)
    return (
      <Card>
        <CardContent className="p-8 text-center text-base text-muted-foreground">
          {t("reports.noMembership")}
        </CardContent>
      </Card>
    );
  return (
    <div className="grid gap-3 sm:grid-cols-3">
      <Stat label="Members" value={String(d.members?.length ?? 0)} />
      <Stat label="Events" value={String(d.events?.length ?? 0)} />
      <Stat label="Visits" value={String(d.visits?.length ?? 0)} />
    </div>
  );
}
function Stat({
