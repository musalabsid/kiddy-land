...
import { CheckCircle2, ScanLine, X, XCircle } from "lucide-react";
...
export function TicketScanner({ kind, enableCamera = false }: { kind: "entry" | "exit"; enableCamera?: boolean }) {
  ...
  const { register, handleSubmit, setValue, getValues, watch, formState: { errors }, clearErrors } = useForm<ScanValues>({ resolver: zodResolver(scanSchema), defaultValues: { code: "" }, mode: "onSubmit", reValidateMode: "onSubmit" });
