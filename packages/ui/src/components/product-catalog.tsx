...
function Field({
  ...
  return (
    <FormField label={label} optional htmlFor={htmlFor}>
      {children}
    </FormField>
  );
}
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-2 border border-border p-3">
      <div className="flex min-w-0 items-center gap-3">
        {product.imageUrl ? (
          <img
            src={`${client.origin}${product.imageUrl}?access_token=${client.getToken() ?? ""}`}
            alt={product.name}
      ...
      <div className="flex flex-wrap items-center gap-3 text-sm">
        ...
        >
          {product.stock} {t("inventory.inStock")} · {t("inventory.threshold")}{" "}
          {product.lowStockThreshold}
        </span>
        <span className="text-xs text-muted-foreground">
          {product.archived ? t("inventory.archived") : t("inventory.active")}
        </span>
        <span className="flex gap-1">
          ...
          <Button
            ...
            onClick={() =>
              product.archived
                ? reactivate.mutate({ id: product.id })
                : archive.mutate({ id: product.id })
            }
          >
            {product.archived ? "Reactivate" : "Archive"}
          </Button>
        </span>
      </div>
    </div>
...
export function ProductCatalog() {
  ...
  const [editFile, setEditFile] = React.useState<File | undefined>();
  const [createFile, setCreateFile] = React.useState<File | undefined>();
  const createPreview = React.useMemo(
    () => (createFile ? URL.createObjectURL(createFile) : undefined),
    [createFile],
  );
  React.useEffect(
    () => () => {
  ...
  const editPreview = React.useMemo(
    () => (editFile ? URL.createObjectURL(editFile) : undefined),
    [editFile],
  );
  React.useEffect(
    () => () => {
        ...
        onSuccess: async (p) => {
          if (editFile) {
            if (editFile.size > 5 * 1024 * 1024) {
              toast.error(t("inventory.imageTooLarge"));
              return;
            }
            try {
              await upload.mutateAsync({ id: p.id, file: editFile });
              toast.success(t("inventory.productUpdatedWithImage"));
            } catch (e) {
              toast.error(
                e instanceof Error
                  ? e.message
  ...
  const submitCreate = handleSubmit(async (values) => {
    try {
      const p = await create.mutateAsync({
        ...
        lowStockThreshold: Number(values.threshold || 0),
      });
      if (createFile) {
        if (createFile.size > 5 * 1024 * 1024) {
          toast.error(t("inventory.imageTooLarge"));
          reset();
        ...
        try {
          await upload.mutateAsync({ id: p.id, file: createFile });
          toast.success(t("inventory.productCreatedWithImage"));
        } catch (e) {
          toast.error(
            e instanceof Error
              ? e.message
              : t("inventory.imageUploadFailedNoImage"),
          );
        }
      }
      reset();
  ...
  return (
    <div className="w-full max-w-6xl px-5 py-8 sm:px-8">
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t("inventory.catalog")}</CardTitle>
          <CardDescription>{t("inventory.catalogDescription")}</CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              ...
              >
                <input
                  ...
                  className={inputCls}
                  {...register("sku")}
                  placeholder={t("inventory.skuExample")}
              ...
              >
                <input
                  ...
                  className={inputCls}
                  {...register("name")}
                  placeholder={t("inventory.nameExample")}
              ...
              >
                <input
                  ...
                  className={inputCls}
                  {...register("barcode")}
                  placeholder={t("inventory.barcodeExample")}
                />
              </FormField>
              <FormField
                label={t("inventory.priceIdr")}
                required
          ...
          <div className="grid gap-2">
            ...
            {editing && (
              <div className="grid items-start gap-3 border border-primary p-3 sm:grid-cols-2 lg:grid-cols-3">
                <p className="border-b pb-2 text-sm font-medium sm:col-span-full">
                  {t("inventory.editing")} {editing.name}
                </p>
                <Field label={t("inventory.sku")} htmlFor="edit-sku">
                  <input
                    id="edit-sku"
                    className={inputCls}
                    value={editSku}
                ...
                <Field label={t("inventory.nameLabel")} htmlFor="edit-name">
                  <input
                    id="edit-name"
                    className={inputCls}
                    value={editName}
                ...
                <Field label={t("inventory.barcode")} htmlFor="edit-barcode">
                  <input
                    id="edit-barcode"
                    className={inputCls}
                    value={editBarcode}
                ...
                <Field label={t("inventory.priceIdr")} htmlFor="edit-price">
                  <input
                    id="edit-price"
                    className={inputCls}
                    type="number"
                ...
                <div className="flex flex-col gap-2">
                  <FormField label={t("inventory.productImage")} optional>
                    <input
                      className={inputCls}
                      type="file"
                      accept="image/*"
                    ...
                    {editPreview ? (
                      ...
                      <img
                        src={`${client.origin}${editing.imageUrl}?access_token=${client.getToken() ?? ""}`}
                        alt={editing.name}
                        className="mt-2 h-24 w-24 border object-cover"
                      />
                    ) : (
                  ...
                  {editFile && (
                    <Button
                      size="sm"
                      variant="ghost"
                      onClick={() => setEditFile(undefined)}
                    >
                      {t("inventory.clearSelected")}
                    </Button>
                  )}
                </div>
                <div className="flex justify-end gap-2 border-t pt-3 sm:col-span-2 lg:col-span-3">
                  <Button
                    size="sm"
                    variant="ghost"
