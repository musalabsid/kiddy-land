...
export function PublicKiosk() {
  ...
  const [ticketData, setTicketData] = React.useState<PublicTicketResult | null>(
    null,
  );
  const [productData, setProductData] = React.useState<PublicProduct | null>(
    null,
  );
  ...
  const products = React.useMemo(
    () => (productsQuery.data ?? []).slice(0, 100),
    [productsQuery.data],
  );
  ...
  const handleDetect = React.useCallback(
    (raw: string) => {
      ...
      const product = products.find(
        (p) => p.barcode === value || p.sku === value || p.id === value,
      );
      if (product) {
      ...
      .then((res) => {
        if (res.data) {
          ...
          setTicketData({
            ...
            remainingMinutes: 0,
          } as unknown as PublicTicketResult);
        }
        stopScan();
      })
      .catch(() => {
  ...
  return (
    <main className="h-dvh overflow-hidden bg-background">
      <div className="mx-auto flex h-dvh max-w-[1280px] gap-6 overflow-hidden p-4 md:p-6">
        {/* Left - Scanner */}
        <div className="flex flex-1 flex-col gap-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <h1 className="text-lg font-semibold tracking-tight">
                {venueQuery.data?.venueName ?? "Kiddy Land"}
              </h1>
              {venueQuery.data?.logoUrl ? (
                <img
                  src={venueQuery.data.logoUrl}
                  alt={venueQuery.data.venueName}
          ...
          <div className="grid gap-3">
              ...
              <Card className="border-primary/20 bg-card shadow-sm">
                <CardHeader className="pb-3">
                  <CardTitle className="flex items-center gap-2 text-base">
                    <Package className="size-4" /> {productData.name}
                  </CardTitle>
                </CardHeader>
                <CardContent className="flex gap-4">
                  {productData.imageUrl ? (
                    <img
                      src={`${client.origin}${productData.imageUrl}?access_token=${client.getToken() ?? ""}`}
                      alt={productData.name}
                      className="size-24 shrink-0 rounded-none border object-cover"
                    />
                  ) : (
                  ...
                  <div className="grid gap-1 text-sm">
                    ...
                    <p className="font-mono font-semibold">
                      {formatIdr(productData.price, locale)}
                    </p>
                  </div>
                </CardContent>
              </Card>
            ...
            {ticketData ? (
              <Card
                className={
                  ticketData.ok
                    ? "border-l-[3px] border-[var(--state-success)] bg-[var(--state-success-bg)]/40"
                    : "border-l-[3px] border-[var(--state-danger)] bg-[var(--state-danger-bg)]/40"
                }
              >
                <CardHeader className="pb-3">
                ...
                <CardContent className="grid gap-2 text-sm">
                    ...
                    <>
                      <div className="flex justify-between gap-4 text-xs">
                        ...
                        <span className="font-medium">
                          {ticketData.ticket.package.name}
                        </span>
                      </div>
                      <div className="flex justify-between gap-4 text-xs">
                        <span className="text-muted-foreground">
                      ...
                      <div className="flex justify-between gap-4 text-xs">
                        ...
                        <span className="font-mono">
                          {ticketData.remainingMinutes} min
                        </span>
                      </div>
                      {ticketData.overtimeMinutes != null &&
                      ticketData.overtimeMinutes > 0 ? (
                        <div className="flex justify-between gap-4 text-xs">
                          <span className="text-muted-foreground">
                            {t("kiosk.overtime")}
                          </span>
                          <span className="font-mono text-[var(--state-warning)]">
                            {ticketData.overtimeMinutes} min
                          </span>
                        </div>
                      ) : null}
                      <div className="flex justify-between gap-4 text-xs">
                        ...
                        <span className="font-mono">
                          {ticketData.ticket.childName ??
                            ticketData.ticket.childId.slice(0, 8)}
                        </span>
                      </div>
                      <div className="flex justify-between gap-4 text-xs">
                        <span className="text-muted-foreground">
                  ...
                  {!ticketData.ticket ? (
                    <p className="text-xs text-muted-foreground">
                      remaining: {ticketData.remainingMinutes} min
                    </p>
                  ) : null}
                </CardContent>
              </Card>
                  ...
                  <div key={product.id} className="flex gap-3 p-3">
                    {product.imageUrl ? (
                      <img
                        ...
                        alt={product.name}
                        className="size-16 shrink-0 rounded-none border object-cover"
                      />
                    ) : (
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="font-mono text-xs font-semibold">
                        {formatIdr(product.price, locale)}
                      </p>
                    </div>
                  </div>
                ))}
      ...
      <div className="border-t md:hidden">
        ...
        <div className="flex gap-3 overflow-x-auto p-3">
          {products.map((product) => (
            <div
              key={product.id}
              className="flex w-32 shrink-0 flex-col gap-2 border p-2"
            >
              {product.imageUrl ? (
                <img
                  src={`${client.origin}${product.imageUrl}?access_token=${client.getToken() ?? ""}`}
                  alt={product.name}
                  className="aspect-square w-full rounded-none border object-cover"
                />
              ) : (
