...
export function MembershipDashboard() {
  ...
  return <div><header className="mb-4"><p className="mb-2 text-xs font-semibold uppercase tracking-[0.2em] text-primary">{t("membership.pageEyebrow")}</p><h2 className="text-2xl font-semibold tracking-tight">{t("membership.pageTitle")}</h2><p className="mt-2 max-w-2xl text-sm text-muted-foreground">{t("membership.pageDescription")}</p></header><Card>
    ...
    <CardContent className="grid gap-4">
      <form className="grid gap-3 border-b pb-4 sm:grid-cols-2 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto]" onSubmit={submit} noValidate>
        <FormField label={t("membership.memberName")} required htmlFor="member-register-name" error={form.formState.errors.name?.message}>
          <input id="member-register-name" className="h-9 border border-input bg-background px-2 text-sm" {...form.register("name")} />
        </FormField>
        <FormField label={t("membership.phoneNumber")} required htmlFor="member-register-phone" error={form.formState.errors.phone?.message}>
          <input id="member-register-phone" className="h-9 border border-input bg-background px-2 text-sm" inputMode="numeric" type="tel" placeholder={t("membership.phonePlaceholder")} {...form.register("phone")} />
        </FormField>
        ...
        <p className="text-xs text-muted-foreground sm:col-span-2">{t("membership.phoneHelp")}</p>
      ...
      <div className="relative">
        <FormField label={t("membership.lookup")} optional htmlFor="membership-search">
          <input id="membership-search" className="h-9 w-full border border-input bg-background px-2 text-sm" placeholder={t("membership.memberSearchPlaceholder")} value={search} onChange={(event) => setSearch(event.target.value)} />
        </FormField>
      ...
      <div className="grid gap-2">
        ...
        {debouncedSearch && searchResults.isSuccess && !visibleMembers.length && <p className="text-sm text-muted-foreground">{t("membership.noMatchingMember")}</p>}
      </div>
      {displayedMember ? <div className="grid gap-2 border p-3"><p><strong>{displayedMember.child.name}</strong> · {displayedMember.member.status} · {displayedMember.member.code}</p><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={printCard}>{t("membership.printCard")}</Button><Button size="sm" variant="outline" onClick={() => reissue.mutate({ id: displayedMember.member.id, reason: "Lost card" })} disabled={reissue.isPending}>{reissue.isPending ? t("membership.reissuing") : t("membership.reissue")}</Button>{isOwner && (displayedMember.member.status === "active" ? <Button size="sm" variant="outline" onClick={() => deactivate.mutate({ id: displayedMember.member.id, reason: "Owner deactivation" })}>{t("membership.deactivate")}</Button> : <Button size="sm" variant="outline" onClick={() => reactivate.mutate({ id: displayedMember.member.id, reason: "Owner reactivation" })}>{t("membership.reactivate")}</Button>)}</div>{reissue.isError ? <p role="alert" className="text-sm text-destructive">{reissue.error.message}</p> : null}</div> : null}
