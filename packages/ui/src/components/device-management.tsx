      ...
      <CardContent>
        <div className="grid gap-6 md:grid-cols-[1fr_auto]">
          <div className="grid gap-4">
            <label className="grid gap-1 text-sm"><span>{t("device.deviceKind")}</span><select className="h-9 border border-input bg-background px-2" value={kind} onChange={(e) => setKind(e.target.value as typeof kind)}><option value="private">{t("device.privateDevice")}</option><option value="public-kiosk">{t("device.publicKiosk")}</option></select></label>
            {kind === "private" && <><FormField label={t("device.employeeName")} required htmlFor="pairing-staff-name" error={touched && !staffName.trim() ? t("device.employeeRequired") : undefined}><input id="pairing-staff-name" className="h-9 border border-input bg-background px-2" placeholder={t("device.employeePlaceholder")} value={staffName} onChange={(e) => setStaffName(e.target.value)} onBlur={() => setTouched(true)} /></FormField><FormField label={t("device.role")} required htmlFor="pairing-staff-role"><select id="pairing-staff-role" className="h-9 border border-input bg-background px-2" value={staffRole} onChange={(e) => setStaffRole(e.target.value as typeof staffRole)}><option value="Cashier">Cashier</option><option value="Staff">Staff</option></select></FormField></>}
