...
export function OwnerLoginScreen() {
  ...
  return <Card><CardHeader><CardTitle>Owner login</CardTitle><CardDescription>Sign in to this host's Owner Dashboard.</CardDescription></CardHeader><CardContent><form className="grid gap-3" onSubmit={(event) => { event.preventDefault(); mutation.mutate(password); }}><PasswordField label="Owner password" value={password} onChange={setPassword} autoComplete="current-password" /><Button type="submit" disabled={mutation.isPending || !password}>Login as owner</Button>{mutation.isError && <p role="alert" className="text-xs text-destructive">{mutation.error.message}</p>}</form></CardContent></Card>;
}
