...
const modes: Array<{
  value: DeviceMode;
  key: "auth.modeCashier" | "auth.modeScanner" | "auth.modeKiosk";
}> = [
  { value: "Cashier", key: "auth.modeCashier" },
  { value: "Scanner", key: "auth.modeScanner" },
  { value: "Public Kiosk", key: "auth.modeKiosk" },
];
const inputCls = "h-9 w-full border border-input bg-background px-2 text-sm";
const pairingSchema = z.object({
  token: z.string().trim().min(1, "Invitation token is required"),
}); // ponytail: message translated via FormField error display
...
export function PairingScreen({
  ...
  const { t } = useLocale();
  const mutation = usePairingMutation();
  const { session } = useSession();
  const navigate = useNavigate();
  const location = useLocation();
  const [mode, setMode] = React.useState<DeviceMode>("Cashier");
  const {
    register,
    handleSubmit,
    setValue,
    formState: { errors },
  } = useForm<PairingValues>({
  ...
  const submit = handleSubmit((values) =>
    mutation.mutate({ token: values.token, mode, origin }),
  );
  const submitToken = (value: string) =>
    mutation.mutate({ token: value.trim(), mode, origin });
  return (
    <Card>
      <CardHeader>
        <CardTitle>{t("auth.pairTitle")}</CardTitle>
        <CardDescription>{t("auth.pairDescription")}</CardDescription>
      </CardHeader>
      <CardContent>
        <form className="grid gap-3" onSubmit={submit} noValidate>
          {enableScanner && (
            <>
              ...
              <p role="note" className="text-xs text-muted-foreground">
                {t("auth.pairCameraNote")} {t("auth.pairMobileLimit")}
              </p>
            </>
          )}
          <FormField
            label={t("auth.pairToken")}
            required
            htmlFor="pair-token"
          ...
          <FormField label={t("auth.deviceMode")} required htmlFor="pair-mode">
            <Select
              id="pair-mode"
              className="h-9"
              value={mode}
          ...
          {mutation.isError && (
            <p role="alert" className="text-xs text-destructive">
              {(() => {
                ...
                if (msg.includes("Public kiosk invitation"))
                  return t("auth.publicKioskOnly");
                if (msg.includes("invalid or expired"))
                  return t("auth.invitationExpired");
                if (msg.includes("revoked or unknown"))
                  return t("auth.deviceRevoked");
                return t("auth.invalidInvitation");
              })()}
            </p>
          )}
        </form>
...
export function OwnerLoginScreen({
  ...
  const {
    ...
    resolver: zodResolver(
      z.object({ password: z.string().min(1, t("auth.passwordRequired")) }),
    ),
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{t("auth.ownerLoginTitle")}</CardTitle>
      ...
      <CardContent>
        <form
          ...
          onSubmit={handleSubmit((values) =>
            mutation.mutate(values.password, { onSuccess }),
          )}
          noValidate
        >
          <FormField
            label={t("auth.ownerPassword")}
            required
          ...
          >
            <span className="relative block">
              <input
                id="owner-password"
                aria-label={t("auth.ownerPassword")}
          ...
          <Button
            type="submit"
            disabled={mutation.isPending || !!errors.password}
          >
            {t("auth.loginAsOwner")}
          </Button>
          {mutation.isError && (
            <p role="alert" className="text-xs text-destructive">
              {authError((mutation.error as Error).message)}
            </p>
          )}
        </form>
      </CardContent>
      ...
      <CardContent>
        ...
        >
          <p className="text-sm text-muted-foreground">
            {t("auth.username")}: <strong>owner</strong>
          </p>
          <FormField
            label={t("auth.ownerPassword")}
            required
            ...
            error={
              errors.password?.message
                ? authError(errors.password.message)
                : undefined
            }
            hint={t("auth.passwordMin8")}
          >
            <input
          ...
          <FormField
            ...
            error={
              errors.confirmation?.message
                ? authError(errors.confirmation.message)
                : undefined
            }
          >
            <input
...
export function AuthScreen({
  ...
  React.useEffect(() => {
    ...
    void navigate({
      to: modeDefaultRoutes[session.device.mode] as never,
      replace: true,
    });
  ...
  React.useEffect(() => {
    ...
    const mismatched =
      (location.pathname === "/kiosk" &&
        session.device.mode !== "Public Kiosk") ||
      ((location.pathname === "/sales" || location.pathname === "/members") &&
        session.device.mode !== "Cashier") ||
      (location.pathname.startsWith("/scanner") &&
        session.device.mode !== "Scanner") ||
      (location.pathname === "/inventory" &&
        session.device.mode !== "Inventory") ||
  ...
  if (bootstrap.data?.ownerDevice && !pairedDevice)
    return (
      <main className="flex min-h-[100dvh] w-full items-center justify-center bg-background p-6">
        <div className="mx-auto grid w-full max-w-sm gap-3">
          <PairingScreen origin={origin} enableScanner={enableScanner} />
          <a
            href="/owner-login"
