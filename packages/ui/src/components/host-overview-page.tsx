...
import { AlertTriangle, CheckCircle2, Clock, LoaderCircle, Package, PlugZap, RefreshCw, Server, ShoppingBag, Ticket, Users, Wallet, WifiOff } from "lucide-react";
...
export function HostOverviewPage({
  ...
  const operatingDate = tickets[0]?.operatingDate ?? (generatedAt ? new Date(generatedAt).toISOString().slice(0, 10) : "");

  const counts = React.useMemo(() => {
    const c: Record<string, number> = { waiting: 0, active: 0, completed: 0, "auto-closed": 0, expired: 0, void: 0 };
    ...
    <main className="min-h-dvh bg-background text-foreground overflow-x-clip">
      <div className="w-full max-w-6xl px-5 py-8 sm:px-8">
        <header className="mb-6">
          <p className="mb-1 text-xs font-semibold uppercase tracking-[0.18em] text-primary">{t("overview.eyebrow")}</p>
          <div className="flex flex-wrap items-end justify-between gap-3">
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              {generatedAt ? (
                <span>
                  {t("overview.generated")} {new Date(generatedAt).toLocaleTimeString(locale === "id" ? "id-ID" : "en-US", { hour: "2-digit", minute: "2-digit" })} · {timezone ?? ""}
                </span>
              ) : null}
              <Button variant="outline" size="sm" className="h-7" onClick={refreshAll} disabled={overview.isFetching || checking}>
                <RefreshCw className={cn("size-3.5", (overview.isFetching || checking) && "animate-spin")} />
                {t("common.refresh")}
              </Button>
            </div>
          </div>
        ...
        {lanIpChanged && (
          <Alert variant="destructive" className="mb-6 border-[var(--state-warning)]/40 bg-[var(--state-warning-bg)]/30 text-[var(--state-warning)]">
            <AlertTriangle className="size-4" />
            <AlertTitle>{t("host.ipChangedTitle") ?? "Host IP changed"}</AlertTitle>
            <AlertDescription className="flex flex-wrap items-center gap-2">
              {`Was ${lanIpChanged.old} → now ${lanIpChanged.cur}. ${t("host.ipChangedDesc")}`}{" "}
              <Button size="sm" variant="outline" onClick={() => setLanIpChanged(null)}>
                {t("common.dismiss") ?? "Dismiss"}
              </Button>{" "}
              <Button size="sm" variant="outline" onClick={() => void check()}>
                <RefreshCw className="size-3" /> {t("common.refresh") ?? "Refresh"}
              </Button>
            </AlertDescription>
          </Alert>
        )}
        ...
        <div className="grid gap-6">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <LiveCard icon={Users} label={t("reports.liveOccupancy")} value={live?.occupancy} loading={overview.isLoading} sub={t("reports.activeSessions")} />
            <LiveCard icon={Wallet} label={t("reports.activeMembers")} value={live?.activeMembers} loading={overview.isLoading} sub={t("reports.notDeactivated")} />
            <LiveCard
              icon={AlertTriangle}
          ...
          <Card>
            <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-1.5">
                <Button variant="outline" size="sm" className="h-7" onClick={refreshAll} disabled={overview.isFetching}>
            ...
            <CardContent className="grid gap-4">
              {overview.isLoading ? (
                ...
                <Alert variant="destructive">
                  ...
                  <AlertDescription className="flex flex-wrap items-center gap-2">
                    {t("reports.checkDates")} <Button size="sm" variant="outline" onClick={refreshAll}>{t("common.refresh")}</Button>
                  </AlertDescription>
                </Alert>
              ) : tickets.length === 0 ? (
                <div className="grid place-items-center gap-2 rounded-lg border border-dashed bg-muted/20 px-6 py-12 text-center">
                  <Ticket className="size-8 text-muted-foreground" />
                  <p className="text-sm font-medium">{t("overview.noTickets")}</p>
                  <p className="max-w-md text-xs leading-5 text-muted-foreground">{t("overview.noTicketsHint")}</p>
                </div>
              ) : (
                <>
                  <div className="flex flex-wrap gap-1.5">
                    {[
                      ...
                      .filter(([key]) => key === "all" || (counts[key as string] ?? 0) > 0)
                      .map(([key, label]) => (
                        <button
                          key={key}
                          ...
                          className={cn(
                            ...
                            statusFilter === key ? "border-primary bg-primary text-primary-foreground" : "border-border bg-background hover:bg-muted",
                          )}
                        >
                          {label} {key !== "all" ? `· ${counts[key] ?? 0}` : `· ${tickets.length}`}
                        </button>
                      ))}
                  </div>
                  ...
                  <div className="overflow-x-auto rounded-lg border">
                    <table className="w-full min-w-[720px] text-sm">
                      <thead className="bg-muted/40 text-xs text-muted-foreground">
                        <tr className="[&>th]:whitespace-nowrap [&>th]:px-3 [&>th]:py-2.5 [&>th]:text-left [&>th]:font-medium">
                          ...
                          <th className="text-right">
                            <span className="inline-flex items-center gap-1">
                              <Clock className="size-3" /> {t("overview.playing")}
                            </span>
                          </th>
                      ...
                      <tbody className="divide-y">
                        {filtered.length === 0 ? (
                          ...
                          filtered.map((tk) => (
                            <TicketRow key={tk.ticketId} ticket={tk} locale={locale} t={t} />
                          ))
                        )}
                      </tbody>
                    </table>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t("overview.showing").replace("{shown}", String(filtered.length)).replace("{total}", String(tickets.length))}
                    {operatingDate ? ` · ${operatingDate}` : ""} {timezone ? `· ${timezone}` : ""}
                  </p>
                </>
              )}
            </CardContent>
    ...
    <Card className={tone === "destructive" ? "border-destructive/30" : ""}>
      <CardContent className="flex items-center gap-4 p-4">
        ...
        <div className="min-w-0">
          ...
          {loading ? <Skeleton className="mt-1 h-7 w-12" /> : <p className="text-3xl font-bold tracking-tight">{value ?? 0}</p>}
...
function TicketRow({ ticket, locale, t }: { ticket: OverviewTicket; locale: "id" | "en"; t: (k: any) => string }) {
  ...
  return (
    <tr className={cn(
      ...
      <td className="px-3 py-2.5">
        <div className="max-w-[14ch] truncate text-sm font-medium leading-tight">{ticket.childName ?? ticket.childId.slice(0, 8)}</div>
        <div className="truncate text-xs text-muted-foreground">{ticket.saleId.slice(0, 8)}</div>
      </td>
      <td className="px-3 py-2.5">
        <div className="max-w-[16ch] truncate text-sm leading-tight">{ticket.packageName}</div>
        <div className="text-xs text-muted-foreground">{ticket.includedMinutes === null ? t("calendar.unlimited") : `${ticket.includedMinutes}m`}</div>
      </td>
      <td className="px-3 py-2.5">
        <span
          className={cn(
            "inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium whitespace-nowrap",
            ticket.status === "active" && "border-primary/30 bg-primary/10 text-primary",
            ticket.status === "waiting" && "border-border bg-muted text-foreground",
            ticket.status === "completed" && "border-[var(--state-success)]/30 bg-[var(--state-success-bg)] text-[var(--state-success)]",
            ticket.status === "auto-closed" && "border-[var(--state-warning)]/30 bg-[var(--state-warning-bg)] text-[var(--state-warning)]",
            (ticket.status === "expired" || ticket.status === "void") && "border-destructive/30 bg-destructive/10 text-destructive",
          )}
        >
        ...
        <div className="text-xs leading-tight">
          <span
            className={cn(
              ...
              ticket.depositStatus === "applied" && "bg-primary/10 text-primary",
              ticket.depositStatus === "refunded" && "bg-[var(--state-success-bg)] text-[var(--state-success)]",
              ticket.depositStatus === "forfeited" && "bg-destructive/10 text-destructive",
            )}
          >
