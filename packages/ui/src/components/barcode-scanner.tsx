...
export function BarcodeScanner({
  ...
  const loopRef = React.useRef<ReturnType<typeof createScanLoop> | undefined>(
    undefined,
  );
  ...
  const [capability, setCapability] = React.useState<
    Awaited<ReturnType<typeof detectCapability>> | undefined
  >(undefined);
  const [focusing, setFocusing] = React.useState(false);
  const [devices, setDevices] = React.useState<MediaDeviceInfo[]>([]);
  const [selectedId, setSelectedId] = React.useState<string | undefined>(() => {
    try {
      return localStorage.getItem("kiddy-land-selected-camera") || undefined;
    } catch {
      return undefined;
    }
  });
  ...
  React.useEffect(() => {
    let alive = true;
    void detectCapability().then((value) => {
  ...
  const start = React.useCallback(async () => {
    ...
    if (!native && !createJsqrDecoder) {
      fail({ kind: "decode-unavailable" });
      return;
    }
    ...
    const pickStream = async () => {
      try {
        ...
        const backCams = cams.filter(
          (d) => /back|environment|rear/i.test(d.label) || !d.label,
        );
        ...
        for (const dev of candidates) {
          try {
            ...
            const isUltraWide =
              ...
              (label.includes("wide") && label.includes("0.5")) ||
              label.includes("0.5x");
  ...
  React.useEffect(() => {
    try {
      if (selectedId)
        localStorage.setItem("kiddy-land-selected-camera", selectedId);
      else localStorage.removeItem("kiddy-land-selected-camera");
    } catch {}
  }, [selectedId]);
  ...
  if (!capability.supported) {
    const note = !capability.secure
      ? errorMessage("insecure")
      : !capability.camera
  ...
  if (status === "error") {
    return (
      <div className={className}>
        <p role="alert" className="text-xs text-destructive">
          {error ? errorMessage(error.kind) : t("auth.scanError")}
        </p>
    ...
    <div className={className}>
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between gap-2">
          <button
            ...
            className="relative z-10 shrink-0 rounded-full border bg-background px-3 py-1.5 text-xs font-medium shadow-sm hover:bg-accent"
          >
            {showPicker ? t("scanner.hideCameras") : t("scanner.showCameras")}{" "}
          ...
          {!autoStart && (
            <button
              type="button"
              onClick={cancel}
              className="inline-flex shrink-0 items-center gap-1 rounded-full border bg-background px-3 py-1.5 text-xs font-medium shadow-sm hover:bg-accent"
            >
              <X className="size-3" /> {t("auth.scanCancel")}
            </button>
          )}
        </div>
        {showPicker && devices.length > 0 && (
          <div className="rounded-lg border bg-muted/30 p-2">
            <div className="flex flex-wrap gap-1.5">
              {devices.map((d) => (
                <button
                  ...
                  className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${selectedId === d.deviceId ? "bg-primary text-primary-foreground shadow" : "border bg-background hover:bg-accent"}`}
                  title={d.label}
                >
                  {d.label
                    ? d.label.slice(0, 24)
                    : `Camera ${d.deviceId.slice(0, 4)}`}{" "}
                  {/front/i.test(d.label)
                    ? "🤳"
                    : /ultra|wide/i.test(d.label)
                      ? "0.5x"
                      : "1x"}
                </button>
              ))}
            ...
            <p className="mt-1.5 text-[11px] text-muted-foreground">
              ...
              {selectedId
                ? devices
                    .find((d) => d.deviceId === selectedId)
                    ?.label?.slice(0, 20) || selectedId.slice(0, 8)
                : "auto (best focus)"}{" "}
              — tap a camera to switch
            </p>
          </div>
        )}
      </div>
      <div
        className={`relative cursor-pointer border ${focusing ? "border-primary ring-2 ring-primary" : "border-border"}`}
        onClick={async () => {
          setFocusing(true);
          try {
            ...
            if (tr) {
              ...
              if (caps?.focusMode?.includes("continuous"))
                ...
                const mid =
                  (caps.focusDistance.min + caps.focusDistance.max) / 2;
                await tr.applyConstraints({
      ...
      >
        <video
          ...
          playsInline
          muted
          className="pointer-events-none aspect-[4/3] w-full border border-border bg-muted object-cover"
          title={t("scanner.tapToFocus")}
        />
        <button
          type="button"
          onClick={async (e) => {
            ...
            try {
              ...
              try {
                const tr2 = streamRef.current?.getVideoTracks()[0] as any;
                await tr2?.applyConstraints({
          ...
          className="absolute inset-x-2 bottom-2 rounded bg-background/90 py-2 text-sm font-medium backdrop-blur hover:bg-background"
        >
          {t("scanner.tapToFocus")} — {t("scanner.holdDistance")}
        </button>
        {focusing && (
