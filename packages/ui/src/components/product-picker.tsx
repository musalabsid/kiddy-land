...
import { useProducts, useClient, type ProductRecord } from "@kiddy-land/client/react";
...
export function ProductPicker({ value, onSelect }: { value?: ProductRecord; onSelect: (product?: ProductRecord) => void }) {
  ...
  const imgSrc = (product: ProductRecord) => product.imageUrl ? `${client.origin}${product.imageUrl}?access_token=${client.getToken() ?? ""}` : undefined;
    ...
    <Combobox value={value?.id ?? null} onValueChange={(id) => onSelect(typeof id === "string" ? products.data?.find((item) => item.id === id) ?? (value?.id === id ? value : undefined) : undefined)} onInputValueChange={setQuery} itemToStringLabel={label}>
      ...
      <ComboboxContent className="w-auto min-w-(--anchor-width) max-w-96">
        <ComboboxList>
          ...
          {products.data?.map((product) => <ComboboxItem key={product.id} value={product.id}><span className="flex items-center gap-2 whitespace-nowrap">{imgSrc(product) ? <img src={imgSrc(product)} alt={product.name} className="size-6 shrink-0 rounded-none object-cover" /> : <span className="size-6 shrink-0 border bg-muted" />}<span>{product.name} · {product.sku}{product.barcode ? ` · ${product.barcode}` : ""}</span></span></ComboboxItem>)}
          {!products.isFetching && !products.data?.length ? <ComboboxEmpty>{t("sale.noProducts")}</ComboboxEmpty> : null}
        </ComboboxList>
      </ComboboxContent>
    </Combobox>
  </FormField>;
