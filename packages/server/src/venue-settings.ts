...
export function createVenueSettingsStore(database: LocalDatabase) {
  const get = (): VenueSettings => {
    ...
    const rawAlertDevices = Array.isArray((raw as any).alertDevices) ? (raw as any).alertDevices.filter((v:string)=> ["Owner","Cashier","Kiosk"].includes(v)) : DEFAULTS.alertDevices;
    return {
      ...
      alertEnabled: typeof (raw as any).alertEnabled === "boolean" ? (raw as any).alertEnabled : DEFAULTS.alertEnabled,
      alertThreshold: Number.isInteger((raw as any).alertThreshold) && (raw as any).alertThreshold >=3 && (raw as any).alertThreshold <=10 ? (raw as any).alertThreshold : DEFAULTS.alertThreshold,
      alertDevices: rawAlertDevices.length ? rawAlertDevices : DEFAULTS.alertDevices,
    };
  };
  const update = (patch: Partial<VenueSettings>): VenueSettings => {
    const current = get();
    const alertThreshold = patch.alertThreshold !== undefined ? (Number.isInteger(patch.alertThreshold) && patch.alertThreshold >=3 && patch.alertThreshold <=10 ? patch.alertThreshold : (()=>{throw new Error("Threshold must be 3-10")})()) : current.alertThreshold;
    const alertDevices = patch.alertDevices !== undefined ? (Array.isArray(patch.alertDevices) ? patch.alertDevices.filter(v=> ["Owner","Cashier","Kiosk"].includes(v as string)) as VenueSettings["alertDevices"] : current.alertDevices) : current.alertDevices;
    const next: VenueSettings = {
      ...
      alertEnabled: patch.alertEnabled !== undefined ? Boolean(patch.alertEnabled) : current.alertEnabled,
