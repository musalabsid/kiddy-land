...
export function createHostRuntime(options: LocalServerOptions): HostRuntime {
  ...
  let diagnostic: HostDiagnostic = {
    state: "starting",
    message: "Local Server starting",
  };
  const build = () =>
    createLocalServer({
      ...options,
      restorePrepared: async (id) => {
        const old = server;
        try {
          await old.stop();
          await old.replacePrepared(id);
          server = build();
          await server.start();
          return { restored: id };
        } catch (error) {
  ...
  const runtime: HostRuntime = {
    ...
    async start() {
      try {
        ...
        diagnostic = {
          state: "fatal",
          message: "Local Server failed to start; restore a Verified Backup",
        };
...
export function getActiveHostRuntime() {
