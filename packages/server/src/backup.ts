...
export function createBackupService(
  ...
  async function load() {
    try {
      ...
      if (
        !Array.isArray(value) ||
        value.some((item) => {
          const record = item as Partial<BackupRecord>;
          return (
            !item ||
            typeof item !== "object" ||
            ...
            !["verified", "failed"].includes(record.status ?? "")
          );
        })
      )
      ...
      if (
        ...
        error.message === "Backup manifest is invalid"
      )
        throw error;
  ...
  function health() {
    const latest = records.find((item) => item.status === "verified");
    return {
      latest,
      ageMs: latest ? Date.now() - latest.createdAt : undefined,
      destination,
      destinationHealthy: Boolean(latest),
    };
  }
  async function save() {
  ...
  async function backup(reason = "on-demand") {
    ...
    const path = join(destination, `${id}.sqlite`);
    ...
    try {
      ...
      if (!verified) throw new Error("Backup integrity check failed");
      ...
      records = [
        record,
        ...records.filter((item) => item.status === "verified"),
      ];
      await save();
      await prune();
  ...
  async function prune(keep = 10) {
    const good = records.filter(
      (item) => item.status === "verified" && item.id !== staged?.id,
    );
    for (const item of good.slice(keep)) {
      try {
  ...
  async function prepareRestore(id: string, confirm: string) {
    ...
    if (!(await verify(selected.destination)))
      throw new Error("Selected backup is no longer valid");
  ...
  async function replacePrepared(id: string) {
    if (
      !staged ||
      staged.id !== id ||
      Date.now() - staged.stagedAt > 10 * 60_000
    ) {
      staged = undefined;
      throw new Error("Restore is not staged or has expired");
    }
    const selected = records.find(
      (item) => item.id === id && item.status === "verified",
    );
    ...
    try {
      ...
      if (!(await verify(temporary)))
        throw new Error("Restored database integrity check failed");
      await rm(`${database.path}-wal`, { force: true });
      await rm(`${database.path}-shm`, { force: true });
      await rename(database.path, rollback);
      try {
        await rename(temporary, database.path);
      } catch (error) {
        try {
          await rename(rollback, database.path);
        } catch {}
        throw error;
      }
      if (!(await verify(database.path))) {
        ...
        try {
          ...
          throw new Error(
            "Restore failed and safety database requires manual recovery",
          );
        }
        throw new Error("Restored database integrity check failed");
      }
  ...
  async function deleteBackup(id: string) {
    ...
    try {
      await rm(removed.destination, { force: true });
    } catch {}
    await save();
    return removed;
  }
