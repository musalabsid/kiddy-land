...
const httpsPort = process.env.KIDDY_LAND_HTTPS === "1" || process.env.KIDDY_LAND_HTTPS === "true" ? Number(process.env.KIDDY_LAND_HTTPS_PORT ?? "43118") : undefined;
const detectedLanIp = detectLanIpv4();
const tlsHosts = (process.env.KIDDY_LAND_TLS_HOSTS ?? "").split(",").map((value) => value.trim()).filter(Boolean);
if (tlsHosts.length === 0 && detectedLanIp) tlsHosts.push(detectedLanIp);
if (process.env.KIDDY_LAND_TLS_HOSTS && !tlsHosts.includes(detectedLanIp ?? "") && detectedLanIp) console.warn(`KIDDY_LAND_TLS_HOSTS set but does not include detected LAN IP ${detectedLanIp}; cert will not cover it`);
...
const webDistCandidate = process.cwd().endsWith("packages/server") ? "../../apps/web/dist" : "apps/web/dist";
const webDist = (() => {
...
const runtime = createHostRuntime({ dataDir, host, port, httpsPort, tlsHosts, webDist });

await runtime.start();
console.log(JSON.stringify({ type: "ready", origin: runtime.server.url, httpsUrl: runtime.server.httpsUrl, tlsFingerprint: runtime.server.tlsFingerprint, lanIp: detectedLanIp, webDist: webDist ?? null }));
