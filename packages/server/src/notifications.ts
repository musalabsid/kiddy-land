...
export function createNotificationService(
  ...
  const routes: Record<NotificationKind, DeviceMode[]> = Object.fromEntries(
    Object.entries(defaultRoutes).map(([kind, modes]) => [kind, [...modes]]),
  ) as Record<NotificationKind, DeviceMode[]>;
  ...
  function configureRoutes(kind: NotificationKind, modes: DeviceMode[]) {
    if (!defaultRoutes[kind] || modes.some((mode) => mode === "Public Kiosk"))
      throw new Error("Public kiosk cannot receive private notifications");
    routes[kind] = [...new Set(modes)];
    return routes[kind];
  }
  function configure(deviceId: string, value: Partial<NotificationSettings>) {
    ...
    const next = {
      soundEnabled: settings.get(deviceId)?.soundEnabled ?? true,
      ...(value.soundEnabled === undefined
        ? {}
        : { soundEnabled: value.soundEnabled }),
    };
    settings.set(deviceId, next);
    return next;
  }
  function publish(
    kind: NotificationKind,
    message: string,
    key = `${kind}:${message}`,
    extra?: { dailyNumber?: string; threshold?: number },
  ) {
    if (notified.has(key)) return;
    notified.add(key);
    const createdAt = Date.now();
    for (const device of identity.devices.values())
      if (!device.revokedAt && routes[kind].includes(device.mode)) {
        ...
        const alert: NotificationAlert = {
          ...
          message,
          createdAt,
          sound: setting.soundEnabled,
          ...extra,
        };
        registry.sendDevice?.(device.id, alert);
      }
  }
  function deviceConnected(deviceId: string) {
    publish(
      "device-connected",
      "A device connected",
  ...
  function check() {
    ...
    for (const session of lifecycle.sessions.values())
      if (session.status === "active") {
        const ticket = lifecycle.findTicket(session.ticketId);
        const minutes = ticket?.package.includedMinutes;
        // venue alert system (alertTimer, gated by alertEnabled) is the primary path — skip when enabled to avoid double-speak
        if (
          !venueSettings?.get().alertEnabled &&
          ticket &&
          minutes !== null &&
          minutes !== undefined &&
          minutesBetween(session.enteredAt, now) >= Math.max(0, minutes - 5)
        )
          publish(
            "five-minute-remaining",
            "A play session has five minutes remaining",
            `five:${session.ticketId}`,
            { dailyNumber: (ticket as any).dailyNumber, threshold: 5 },
          );
      }
    for (const event of lifecycle.events)
      if (event.type === "expired" && event.at >= startedAt)
        publish(
          "ticket-expired",
          "A ticket expired",
          `expired:${event.ticketId}:${event.at}`,
        );
    for (const item of inventory.list(undefined, false)) {
      const low = item.stock <= item.lowStockThreshold;
      if (low && !lowStockNotified.has(item.id)) {
        lowStockNotified.add(item.id);
        publish(
          "inventory-low",
          "Inventory is low",
          `inventory:${item.id}:${item.stock}`,
        );
      }
