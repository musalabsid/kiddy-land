...
export type Enrollment = {
  ...
  expiresAt: number;
  usedAt?: number;
  staff?: { username: string; role: Role; name: string };
};
...
function sameHost(a: string, b: string): boolean {
  try {
    ...
    const privateLan = (h: string) =>
      /^192\.168\.|^10\.|^172\.(1[6-9]|2\d|3[01])\./.test(h);
    return (
      (loopback(hostA) || privateLan(hostA)) &&
      (loopback(hostB) || privateLan(hostB))
    );
  } catch {
    return false;
  }
...
export function createIdentityStore(initial?: {
  ownerPassword?: string;
  events?: Partial<IdentityEvents>;
  database?: LocalDatabase;
}) {
  const users = new Map<string, StaffUser>();
  const devices = new Map<string, PairedDevice>();
  const database = initial?.database;
  const persistUser = (user: StaffUser) =>
    database?.db.run(
      "INSERT INTO staff_users(id, username, role, password_hash, created_at, display_name) VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET username=excluded.username, role=excluded.role, password_hash=excluded.password_hash, display_name=excluded.display_name",
      [
        user.id,
        user.username,
        user.role,
  ...
  const persistDevice = (device: PairedDevice) =>
    database?.db.run(
      "INSERT INTO paired_devices(id, mode, kind, revoked_at, created_at, employee_name) VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET mode=excluded.mode, kind=excluded.kind, revoked_at=excluded.revoked_at, employee_name=excluded.employee_name",
      [
        device.id,
        device.mode,
        device.kind,
  ...
  const persistSession = (session: Session) =>
    database?.db.run(
      "INSERT INTO sessions(token, device_id, user_id, created_at, expires_at) VALUES (?, ?, ?, ?, ?) ON CONFLICT(token) DO UPDATE SET device_id=excluded.device_id, user_id=excluded.user_id, expires_at=excluded.expires_at",
      [
        session.token,
        session.deviceId,
        session.userId ?? null,
  ...
  const deleteSession = (token: string) =>
    database?.db.run("DELETE FROM sessions WHERE token = ?", [token]);
  const deleteSessionsByDevice = (deviceId: string) =>
    database?.db.run("DELETE FROM sessions WHERE device_id = ?", [deviceId]);
  try {
    database?.db.run("ALTER TABLE staff_users ADD COLUMN display_name TEXT");
  } catch {}
  const storedUsers = database?.db
    .query(
      "SELECT id, username, role, password_hash AS passwordHash, display_name AS displayName FROM staff_users",
    )
    .all() as Array<StaffUser> | undefined;
  storedUsers?.forEach((user) => users.set(user.id, user));
  const owner: StaffUser = [...users.values()].find(
    (user) => user.username === "owner",
  ) ?? {
    id: id("user"),
    username: "owner",
    role: "Owner",
    passwordHash: hashPassword(initial?.ownerPassword ?? "change-me"),
  };
  ...
  try {
    database?.db.run(
      "ALTER TABLE paired_devices ADD COLUMN employee_name TEXT",
    );
  } catch {}
  const storedDevices = database?.db
    .query(
      "SELECT id, mode, kind, revoked_at AS revokedAt, created_at AS createdAt, employee_name AS employeeName FROM paired_devices",
    )
    .all() as PairedDevice[] | undefined;
  try {
    database?.db.run(
  ...
  const storedSessions = database?.db
    .query(
      "SELECT token, device_id AS deviceId, user_id AS userId, created_at AS createdAt, expires_at AS expiresAt FROM sessions WHERE expires_at > ?",
    )
    .all(Date.now()) as Session[] | undefined;
  ...
  storedDevices?.forEach((device) => {
    ...
    if (m === "Entrance Scanner" || m === "Exit Scanner")
      (device as { mode: string }).mode = "Scanner";
    devices.set(device.id, device);
  });

  function isBootstrapped() {
    return [...devices.values()].some((device) => !device.revokedAt);
  }
  function ownerDevice() {
    return [...devices.values()].find(
      (device) => device.mode === "Owner Dashboard" && !device.revokedAt,
    );
  }
  ...
  function createEnrollment(
    ...
    const invitation: Enrollment = {
      ...
      staff: staff
        ? {
            username: `staff-${token().slice(0, 8)}`,
            role: staff.role,
            name: staff.name,
          }
        : undefined,
    ...
    return {
      ...
      qrPayload: JSON.stringify({
        ...
        kind,
        ...(staff ? { staff: { name: staff.name, role: staff.role } } : {}),
      }),
    };
  }
  function normalizeMode(mode: string): DeviceMode {
    return (
      mode === "Entrance Scanner" || mode === "Exit Scanner" ? "Scanner" : mode
    ) as DeviceMode;
  }
  ...
  function pair(enrollmentToken: string, mode: DeviceMode, origin?: string) {
    ...
    if (!invitation || invitation.usedAt || invitation.expiresAt <= Date.now())
      throw new Error("Enrollment invitation is invalid or expired");
    ...
    if (origin !== undefined && !sameHost(origin, invitation.origin))
      throw new Error("Enrollment invitation is invalid or expired");
    if (invitation.kind === "public-kiosk" && mode !== "Public Kiosk") {
      throw new Error(
        `Public kiosk invitation can only be used for Public Kiosk device`,
      );
    }
    if (invitation.staff && !isCompatible(invitation.staff.role, mode)) {
      throw new Error(
        `Role ${invitation.staff.role} cannot use ${mode} device — ${mode} requires ${allowedForMode(mode)} role`,
      );
    }
    ...
    const device: PairedDevice = {
      ...
      createdAt: Date.now(),
      employeeName: invitation.staff?.name,
    };
    devices.set(device.id, device);
    persistDevice(device);
    ...
    if (invitation.staff) {
      const user: StaffUser = {
        id: id("user"),
        username: invitation.staff.username,
        role: invitation.staff.role,
        passwordHash: hashPassword(invitation.token),
        displayName: invitation.staff.name,
      };
    ...
    const session =
      invitation.kind === "public-kiosk" || invitation.staff
        ? createSession(device.id, userId)
        : undefined;
  ...
  function login(deviceId: string, username: string, password: string) {
    ...
    if (device && !isCompatible(user.role, device.mode as string)) {
      throw new Error(
        `Role ${user.role} cannot use ${device.mode} device — ${device.mode} requires ${allowedForMode(device.mode as string)} role`,
      );
    }
  ...
  function can(
    identity: NonNullable<ReturnType<typeof authenticate>>,
    capability: string,
  ) {
    const role = identity.user?.role ?? "Public";
    return (
      capabilities[role].has(capability) &&
      modeCapabilities[identity.device.mode].has(capability)
    );
  }
