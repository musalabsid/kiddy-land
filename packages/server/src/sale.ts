...
type ProductSnapshot = InventoryProductSnapshot & { membershipDiscount: number }
...
export type TicketLineInput = { kind?: "ticket"; childId: string; childName?: string; packageId: string; memberId?: string; paymentConfirmed?: boolean };
...
export type SaleRecord = { id: string; idempotencyKey: string; cashierId: string; operatingDate: string; paymentMethod: PaymentMethod; paymentConfirmedAt: number; status: SaleStatus; tickets: TicketRecord[]; deposits: DepositRecord[]; receipt: ReceiptRecord; lines: ReceiptLine[]; total: number; createdAt: number; corrections?: SaleCorrection[] };
...
export function createSaleStore(calendar: CalendarStore, database?: LocalDatabase, inventory?: InventoryStore, membership?: MembershipStore) {
  ...
  function completeSale(input: { idempotencyKey: string; cashierId: string; operatingDate: string; at?: number; lines: SaleLineInput[]; paymentMethod: PaymentMethod; locale?: "id" | "en" }) {
    ...
    const sale: SaleRecord = { id: saleId, idempotencyKey: input.idempotencyKey, cashierId: input.cashierId, operatingDate: input.operatingDate, paymentMethod: input.paymentMethod, paymentConfirmedAt: Date.now(), status: "completed", tickets, deposits, receipt, lines: receiptLines, total, createdAt: Date.now(), corrections: [] };
    sales.set(sale.id, sale); idempotency.set(input.idempotencyKey, sale); persist();
    if (membership) {
      ticketLines.forEach((line, index) => { if (line.memberId && ticketDiscounts[index]! > 0) membership.event({ type: "discount-applied", memberId: line.memberId, childId: line.childId, saleId, lineId: tickets[index]!.id, amount: ticketDiscounts[index], actorId: input.cashierId }); });
      productLines.forEach((line, index) => { const amount = productSnapshots[index]!.membershipDiscount; if (line.memberId && amount > 0) membership.event({ type: "discount-applied", memberId: line.memberId, childId: membership.find(line.memberId)!.child.id, saleId, lineId: `${saleId}_product_${index + 1}`, amount, actorId: input.cashierId }); });
      membership.persist();
    }
    return sale;
  }
