...
export function createMembershipStore(database?: LocalDatabase) {
  ...
  function register(input: { name: string; phone?: string }, actorId: string) { if (!input.name?.trim()) throw new Error("Child name is required"); const phone = input.phone ? normalizePhone(input.phone) : undefined; if (!phone || !/^08\d{8,}$/.test(phone)) throw new Error("Phone must start with 08 and contain at least 10 digits"); if (state.children.some((child) => child.phone && normalizePhone(child.phone) === phone)) throw new Error("Phone number is already registered"); if (state.children.some((child) => normalize(child.name) === normalize(input.name) && state.members.some((member) => member.childId === child.id && member.status === "active"))) throw new Error("Child already has active member"); const child: ChildRecord = { id: id("child"), name: input.name.trim(), phone, createdAt: Date.now(), updatedAt: Date.now() }; const member: MemberRecord = { id: id("member"), childId: child.id, status: "active", code: code(), cards: [], createdAt: Date.now(), updatedAt: Date.now() }; member.cards.push({ code: member.code, issuedAt: member.createdAt }); state.children.push(child); state.members.push(member); event({ type: "registered", memberId: member.id, childId: child.id, actorId }); persist(); return { member, child }; }
  ...
  function search(query: string) { const needle = normalize(query); return state.members.filter((member) => { const child = state.children.find((item) => item.id === member.childId); return child && (normalize(child.name).includes(needle) || member.code.toLocaleLowerCase().includes(needle)); }).map((member) => ({ member, child: state.children.find((item) => item.id === member.childId)! })).slice(0, 5); }
  ...
  function reissue(memberId: string, reason: string, actorId: string) { const found = find(memberId); if (!found || !reason.trim()) throw new Error("Member unavailable"); if (state.events.filter((item) => item.memberId === memberId && item.type === "reissued").length >= MAX_REISSUES) throw new Error("Member reissue limit reached"); const at = Date.now(); const prior = found.member.code; const next = code(); const card = found.member.cards.find((item) => item.code === prior); if (card) { card.revokedAt = at; card.reason = reason; } found.member.code = next; found.member.cards.push({ code: next, issuedAt: at }); found.member.updatedAt = at; event({ type: "reissued", memberId, childId: found.member.childId, reason, actorId }); persist(); return found; }
