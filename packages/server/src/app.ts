...
export function createApp(
  ...
  if (reports) {
    ...
    const exportReport = (kind: "financial" | "playground" | "inventory" | "membership", format: "csv" | "pdf", c: any) => { if (!owner(c)) return c.json({ error: "Forbidden" }, 403); try { const value = reports[kind](query(c)); const venueName = venueSettings?.get().venueName ?? "Kiddy Land"; const body = format === "csv" ? reportCsv(value) : reportPdf(value, venueName); return new Response(body, { headers: { "Content-Type": format === "csv" ? "text/csv; charset=utf-8" : "application/pdf", "Content-Disposition": `attachment; filename="${kind}-report.${format}"` } }); } catch { return c.json({ error: "Report unavailable" }, 400); } };
