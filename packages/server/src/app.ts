...
export function createApp(
  ...
  const rateLimit = (c: Context, max: number, windowMs: number): boolean => {
    ...
    if (!entry || entry.resetAt <= now) {
      authLimits.set(ip, { count: 1, resetAt: now + windowMs });
      return true;
    }
    entry.count += 1;
    ...
    cors({
      origin: (origin) => {
        try {
          ...
          const trusted =
            /^(localhost|127\.0\.0\.1|\[::1\]|192\.168\.\d+\.\d+)$/.test(
              hostname,
            ) ||
            (process.env.KIDDY_LAND_TRUSTED_ORIGINS ?? "")
              .split(",")
  ...
  app.use("*", async (c, next) => {
    if (
      c.req.method !== "GET" &&
      getHealth().writeBlocked &&
      !c.req.path.includes("/restore") &&
      !c.req.path.startsWith("/health") &&
      !c.req.path.startsWith("/ready")
    )
      return c.json(
        {
          error: "Server is in recovery mode",
          diagnostic: getHealth().diagnostic,
        },
        503,
      );
    await next();
  ...
  app.get("/backups", (c) => {
    const current = identity?.authenticate(
      c.req.header("Authorization")?.replace(/^Bearer /, ""),
    );
    if (
      !current ||
      current.user?.role !== "Owner" ||
      !identity?.can(current, "read")
    )
  ...
  app.delete("/backups/:id", async (c) => {
    ...
    if (
      ...
      return c.json({ error: "Forbidden" }, 403);
    try {
      return c.json(await backups?.deleteBackup(c.req.param("id")));
    } catch (error) {
      return c.json(
    ...
    return c.json(
      current
        ? {
            ...
            alertEndedEnabled: current.alertEndedEnabled,
            alertEndedTextDefault: current.alertEndedTextDefault,
            alertEndedTextName: current.alertEndedTextName,
          }
        : {
            venueName: "Kiddy Land",
            logoUrl: null,
  ...
  app.post("/backups/:id/restore/stage", async (c) => {
    ...
    try {
      setRecoveryBlocked?.(
        true,
        "Restore staging. Writes are blocked until restore completes or the server restarts.",
      );
      const body = await c.req.json<{ confirmation: string }>();
      const result = await backups?.prepareRestore(
        c.req.param("id"),
        body.confirmation,
      );
      ...
      return c.json(
        {
          error:
            error instanceof Error ? error.message : "Restore staging failed",
        },
        409,
      );
    }
  ...
  app.post("/backups/:id/restore", async (c) => {
    ...
    if (
      ...
      return c.json({ error: "Forbidden" }, 403);
    try {
      return c.json(await restorePrepared?.(c.req.param("id")));
    } catch (error) {
      setRecoveryBlocked?.(
        true,
        error instanceof Error
          ? error.message
          : "Restore failed. Restore the safety backup.",
      );
      return c.json(
  ...
  app.get("/ready", (c) => {
    ...
    return c.json(
      {
        ...health,
        httpsUrl: realtime?.httpsUrl?.() ?? undefined,
        lanIp: realtime?.lanIp?.() ?? undefined,
      },
  ...
  app.get("/overview", (c) => {
    ...
    if (
      ...
      return c.json({ error: "Forbidden" }, 403);
    if (!reports || !sales || !lifecycle || !calendar)
    ...
    const minutesBetween = (a: number, b: number) =>
      Math.max(0, Math.floor((b - a) / 60_000));
    const tickets: Array<{
      ticketId: string;
      code: string;
      dailyNumber: string;
    ...
    for (const sale of sales.sales.values()) {
      ...
      for (const ticket of sale.tickets) {
        ...
        const threshold =
          (ticket.package as unknown as { overtimeThreshold?: number })
            .overtimeThreshold ?? 5;
        ...
        if (!session) {
          ...
          remainingMinutes =
            included === null ? null : Math.max(0, included - playingMinutes);
          overtimeMinutes =
            included === null
              ? 0
              : Math.max(0, playingMinutes - included - threshold);
        } else {
          enteredAt = session.enteredAt;
          exitedAt = session.exitedAt;
          playingMinutes = exitedAt
            ? minutesBetween(session.enteredAt, exitedAt)
            : minutesBetween(session.enteredAt, now);
          remainingMinutes = 0;
        ...
        tickets.push({
          ...
          code: ticket.code,
          dailyNumber:
            (ticket as unknown as { dailyNumber?: string }).dailyNumber ??
            ticket.code,
    ...
    tickets.sort(
      (a, b) =>
        ...
        b.playingMinutes - a.playingMinutes,
    );
    return c.json({
      health,
      live,
  ...
  if (reports) {
    ...
    const query = (c: any) => ({
      ...
      to: c.req.query("to"),
      cashierId: c.req.query("cashierId"),
      paymentMethod: c.req.query("paymentMethod"),
      packageId: c.req.query("packageId"),
      productId: c.req.query("productId"),
      memberId: c.req.query("memberId"),
    });
    const report = (kind: string, c: any) => {
      if (!owner(c)) return c.json({ error: "Forbidden" }, 403);
      try {
        return c.json((reports as any)[kind](query(c)));
      } catch (error) {
    ...
    app.get("/reports/financial", (c) => report("financial", c));
    app.get("/reports/playground", (c) => report("playground", c));
    app.get("/reports/inventory", (c) => report("inventory", c));
    app.get("/reports/membership", (c) => report("membership", c));
    app.get("/reports/live", (c) => {
      if (!owner(c)) return c.json({ error: "Forbidden" }, 403);
      return c.json(reports.live());
    });
    ...
    if (sales && lifecycle && calendar) {
      app.get("/overview/tickets", (c) => {
        ...
        if (!current || !identity?.can(current, "read"))
        ...
        for (const sale of sales.sales.values()) {
          ...
          for (const ticket of sale.tickets) {
            ...
            if (session?.status === "active" && session.enteredAt) {
              ...
              if (included !== null) {
                const threshold =
                  (ticket.package as any).overtimeThreshold ?? 5;
    ...
    const exportReport = (
      kind: "financial" | "playground" | "inventory" | "membership",
      format: "csv" | "pdf",
      c: any,
    ) => {
      if (!owner(c)) return c.json({ error: "Forbidden" }, 403);
      try {
        const value = reports[kind](query(c));
        const venue = venueSettings?.get() ?? {
          venueName: "Kiddy Land",
          logoUrl: null,
        };
        const body =
          format === "csv"
            ? reportCsv(value)
            : reportPdf(value, venue.venueName, venue.logoUrl as string | null);
        return new Response(body, {
          headers: {
            "Content-Type":
              format === "csv" ? "text/csv; charset=utf-8" : "application/pdf",
            "Content-Disposition": `attachment; filename="${kind}-report.${format}"`,
          },
        });
      } catch {
    ...
    for (const kind of [
      ...
      app.get(`/reports/${kind}.csv`, (c) => exportReport(kind, "csv", c));
      app.get(`/reports/${kind}.pdf`, (c) => exportReport(kind, "pdf", c));
    }
  }
  ...
  if (calendar) {
    app.get("/calendar/config", (c) => {
      ...
      return c.json({
        ...
        overrides: [...calendar.overrides.values()],
        packages: [...calendar.packages.values()],
  ...
  if (lifecycle) {
    app.post("/public/tickets/validate", async (c) => {
      ...
      if (
        !current ||
        current.device.mode !== "Public Kiosk" ||
        !identity?.can(current, "public:read")
      )
        return c.json({ error: "Forbidden" }, 403);
      try {
        return c.json(
          lifecycle.publicTicket(
            (await c.req.json<{ code?: string }>()).code ?? "",
          ),
        );
      } catch {
        return c.json({ error: "Ticket validation unavailable" }, 409);
      }
    });
    app.post("/tickets/scan/entry", async (c) => {
      ...
      if (!current || !identity?.can(current, "ticket:admit"))
        return c.json({ error: "Forbidden" }, 403);
      const result = lifecycle.admit(
        (await c.req.json<{ code?: string }>()).code ?? "",
      );
      if (realtime)
        publishReportEvent(realtime.registry, {
          type: "report-changed",
          source: "session",
        });
      return c.json(result);
    ...
    app.post("/tickets/scan/exit", async (c) => {
      ...
      if (!current || !identity?.can(current, "ticket:exit"))
        return c.json({ error: "Forbidden" }, 403);
    ...
    app.post("/tickets/close", async (c) => {
      ...
      const result = lifecycle.close(body.date, body.at ?? Date.now());
      if (realtime)
        publishReportEvent(realtime.registry, {
          type: "report-changed",
          source: "session",
    ...
    app.post("/tickets/:id/collect-charge", async (c) => {
      ...
      if (
        !current ||
        current.device.mode !== "Cashier" ||
        !identity?.can(current, "write")
      )
        return c.json({ error: "Forbidden" }, 403);
      try {
        const body = await c.req.json<{
        ...
        const result = lifecycle.collectOutstanding(
          c.req.param("id"),
          body.amount,
          body.paymentMethod,
          current.user?.id ?? "cashier",
        );
        if (realtime)
          publishReportEvent(realtime.registry, {
            type: "report-changed",
        ...
        return c.json({ error: "Charge cannot be collected" }, 409);
      }
    });
    app.post("/tickets/:id/waive-charge", async (c) => {
      ...
      try {
        ...
        const result = lifecycle.waiveOutstanding(
          c.req.param("id"),
          current.user.role,
          body.reason,
        );
        ...
        return c.json({ error: "Charge cannot be waived" }, 409);
      }
    });
    app.post("/tickets/:id/refund-deposit", async (c) => {
      ...
      if (
        !current ||
        (!identity?.can(current, "write") &&
          !identity?.can(current, "ticket:exit"))
      )
        return c.json({ error: "Forbidden" }, 403);
      try {
        const result = lifecycle.refundDeposit(
          c.req.param("id"),
          current.user?.role ?? "Cashier",
        );
        if (realtime)
          publishReportEvent(realtime.registry, {
            type: "report-changed",
        ...
        return c.json({ error: "Deposit cannot be refunded" }, 409);
      }
    });
    app.post("/tickets/recover", async (c) => {
      ...
      if (!current || !identity?.can(current, "ticket:admit"))
        return c.json({ error: "Forbidden" }, 403);
      try {
        const body = await c.req.json<{ code: string; childId: string }>();
        const result = lifecycle.recover(body.code, body.childId);
        if (realtime)
          publishReportEvent(realtime.registry, {
            type: "report-changed",
            source: "session",
        ...
        return c.json({ error: "Recovery verification failed" }, 409);
      }
    });
  }
  ...
  async function saveProductImage(
    ...
    const { mkdir, writeFile, readdir, unlink } =
      ...
      file.type === "image/png"
        ...
        : file.type === "image/webp"
          ...
          : file.type === "image/gif"
            ...
            : ".jpg";
    try {
      for (const f of await readdir(productImageDir))
        if (f.startsWith(productId + "."))
          await unlink(join(productImageDir, f));
    } catch {}
    await writeFile(
      join(productImageDir, productId + ext),
  ...
  if (inventory) {
    app.get("/products/:id/image", async (c) => {
      ...
      if (
        ...
        return c.json(
          {
            error:
              "Unauthorized — add Authorization: Bearer <token> or ?access_token=<token>",
          },
          401,
        );
      ...
      try {
        for (const f of await readdir(productImageDir))
          if (f.startsWith(c.req.param("id") + ".")) {
            const buf = await readFile(join(productImageDir, f));
            const ct = f.endsWith(".png")
              ? "image/png"
              : f.endsWith(".webp")
                ? "image/webp"
                : f.endsWith(".gif")
    ...
    app.post("/products/:id/image", async (c) => {
      ...
      try {
        ...
        const url = await saveProductImage(c.req.param("id"), file);
        const item = (inventory as any).setImage(c.req.param("id"), url);
        if (realtime)
          publishReportEvent(realtime.registry, {
            type: "report-changed",
            source: "inventory",
    ...
    app.delete("/products/:id/image", async (c) => {
      ...
      try {
        ...
        const item = (inventory as any).setImage(c.req.param("id"), undefined);
        if (realtime)
          publishReportEvent(realtime.registry, {
            type: "report-changed",
            source: "inventory",
        ...
        return c.json({ error: "Image cannot be removed" }, 400);
      }
    });
    app.get("/public/products", (c) => {
      ...
      return c.json(
        inventory
          .list(c.req.query("search"), false, 100)
          .map(({ id, sku, name, price, barcode, imageUrl }) => ({
            id,
            sku,
            name,
            price,
    ...
    app.get("/products", (c) => {
      ...
      return c.json(
        inventory.list(
          c.req.query("search"),
          current.user?.role === "Owner",
          c.req.query("search") ? undefined : 10,
        ),
      );
    });
    app.get("/products/:id", (c) => {
      ...
      if (!current || !identity?.can(current, "read"))
        return c.json({ error: "Unauthorized" }, 401);
      const item = inventory.products.get(c.req.param("id"));
      return item ? c.json(item) : c.json({ error: "Product not found" }, 404);
    });
    app.post("/products", async (c) => {
      ...
      if (
        ...
        return c.json({ error: "Forbidden" }, 403);
      try {
        const result = inventory.create(await c.req.json(), current.user.id);
        if (realtime)
          publishReportEvent(realtime.registry, {
            type: "report-changed",
            source: "inventory",
        ...
        return c.json({ error: "Product cannot be created" }, 409);
      }
    });
    app.patch("/products/:id", async (c) => {
      ...
      if (
        ...
        return c.json({ error: "Forbidden" }, 403);
      try {
        const result = inventory.update(c.req.param("id"), await c.req.json());
        if (realtime)
          publishReportEvent(realtime.registry, {
            type: "report-changed",
            source: "inventory",
        ...
        return c.json({ error: "Product cannot be updated" }, 409);
      }
    });
    app.post("/products/:id/archive", (c) => {
      const current = identity?.authenticate(
        c.req.header("Authorization")?.replace(/^Bearer /, ""),
      );
      if (
        ...
        return c.json({ error: "Forbidden" }, 403);
      try {
        const result = inventory.archive(c.req.param("id"));
        if (realtime)
          publishReportEvent(realtime.registry, {
            type: "report-changed",
            source: "inventory",
        ...
        return c.json({ error: "Product cannot be archived" }, 409);
      }
    });
    app.post("/products/:id/reactivate", (c) => {
      const current = identity?.authenticate(
        c.req.header("Authorization")?.replace(/^Bearer /, ""),
      );
      if (
        ...
        return c.json({ error: "Forbidden" }, 403);
      try {
        const result = inventory.reactivate(c.req.param("id"));
        if (realtime)
          publishReportEvent(realtime.registry, {
            type: "report-changed",
            source: "inventory",
        ...
        return c.json({ error: "Product cannot be reactivated" }, 409);
      }
    });
    app.post("/inventory/intake", async (c) => {
      const current = identity?.authenticate(
        c.req.header("Authorization")?.replace(/^Bearer /, ""),
      );
      if (!current || !identity?.can(current, "inventory:write"))
        return c.json({ error: "Forbidden" }, 403);
      try {
        const body = await c.req.json<{
          productId: string;
        ...
        return c.json({ error: "Stock intake cannot be recorded" }, 409);
      }
    });
    app.post("/inventory/counts", async (c) => {
      ...
      if (!current || !identity?.can(current, "inventory:write"))
        return c.json({ error: "Forbidden" }, 403);
      try {
        const body = await c.req.json<{ productId: string; counted: number }>();
        const result = inventory.submitCount(
          body.productId,
          body.counted,
          current.user?.id ?? "inventory",
        );
        if (current.user?.role === "Owner") {
          try {
            const approved = inventory.approveCount(result.id, current.user.id);
            if (realtime)
              publishReportEvent(realtime.registry, {
                type: "report-changed",
                source: "inventory",
        ...
        return c.json({ error: "Stock count cannot be submitted" }, 409);
      }
    });
    app.post("/inventory/counts/:id/approve", (c) => {
      const current = identity?.authenticate(
        c.req.header("Authorization")?.replace(/^Bearer /, ""),
      );
      if (
        ...
        return c.json({ error: "Forbidden" }, 403);
      try {
        const result = inventory.approveCount(
          c.req.param("id"),
          current.user.id,
        );
        if (realtime)
        ...
        return c.json({ error: "Stock count cannot be approved" }, 409);
      }
    });
    app.get("/inventory/movements", (c) => {
      const current = identity?.authenticate(
        c.req.header("Authorization")?.replace(/^Bearer /, ""),
      );
      if (!current || !identity?.can(current, "read"))
        return c.json({ error: "Unauthorized" }, 401);
      return c.json(inventory.movements);
    });
    app.get("/inventory/low-stock", (c) => {
      const current = identity?.authenticate(
        c.req.header("Authorization")?.replace(/^Bearer /, ""),
      );
      if (!current || !identity?.can(current, "read"))
    ...
    app.get("/inventory/exceptions", (c) => {
      const current = identity?.authenticate(
        c.req.header("Authorization")?.replace(/^Bearer /, ""),
      );
      if (!current || !identity?.can(current, "read"))
        return c.json({ error: "Unauthorized" }, 401);
      return c.json(inventory.exceptions);
    });
    app.get("/inventory/counts", (c) => {
      const current = identity?.authenticate(
        c.req.header("Authorization")?.replace(/^Bearer /, ""),
      );
      if (!current || !identity?.can(current, "read"))
        return c.json({ error: "Unauthorized" }, 401);
      return c.json(inventory.counts);
    });
  }

  if (membership) {
    app.get("/members/search", (c) => {
      const current = identity?.authenticate(
        c.req.header("Authorization")?.replace(/^Bearer /, ""),
      );
      if (!current || !identity?.can(current, "read"))
      ...
      return c.json(membership.search(query));
    });
    app.get("/members", (c) => {
      const current = identity?.authenticate(
        c.req.header("Authorization")?.replace(/^Bearer /, ""),
      ...
      if (!current || !identity?.can(current, "read"))
        return c.json({ error: "Forbidden" }, 403);
      return c.json(membership.list());
    });
    app.get("/members/:code", (c) => {
      const current = identity?.authenticate(
        c.req.header("Authorization")?.replace(/^Bearer /, ""),
      );
      if (!current || !identity?.can(current, "read"))
        return c.json({ error: "Unauthorized" }, 401);
      const found = membership.findByCode(c.req.param("code"));
      return found ? c.json(found) : c.json({ error: "Member not found" }, 404);
    });
    app.post("/members", async (c) => {
      ...
      if (
        ...
        return c.json({ error: "Forbidden" }, 403);
      try {
        const result = membership.register(
          await c.req.json(),
          current.user?.id ?? "cashier",
        );
        if (realtime)
        ...
        return c.json({ error: "Member cannot be registered" }, 409);
      }
    });
    app.post("/members/:id/reissue", async (c) => {
      const current = identity?.authenticate(
        c.req.header("Authorization")?.replace(/^Bearer /, ""),
      );
      if (
        ...
        !(
          (current.device.mode === "Cashier" &&
            identity?.can(current, "write")) ||
          (current.user?.role === "Owner" && identity?.can(current, "admin"))
        )
      )
        return c.json({ error: "Forbidden" }, 403);
      try {
        const body = await c.req.json<{ reason: string }>();
        const result = membership.reissue(
          c.req.param("id"),
          body.reason,
        ...
        return c.json(
          {
            error:
              error instanceof Error
                ? error.message
                : "Member code cannot be reissued",
          },
          409,
        );
      }
    ...
    for (const [action, status] of [
      ...
      app.post(`/members/:id/${action}`, async (c) => {
        ...
        try {
          const body = await c.req.json<{ reason: string }>();
          const result = membership.setStatus(
            c.req.param("id"),
            status,
            body.reason,
            current.user.id,
          ...
          return c.json({ error: "Member status cannot be changed" }, 409);
        }
      });
    app.get("/members/:id/history", (c) => {
      const current = identity?.authenticate(
        c.req.header("Authorization")?.replace(/^Bearer /, ""),
      );
      const allowed =
        current &&
        identity?.can(current, "read") &&
        (current.device.mode === "Cashier" || current.user?.role === "Owner");
      if (!allowed) return c.json({ error: "Forbidden" }, 403);
      return c.json(membership.history(c.req.param("id")));
    });
    app.get("/membership/events", (c) => {
      const current = identity?.authenticate(
        c.req.header("Authorization")?.replace(/^Bearer /, ""),
      );
      if (
        ...
        return c.json({ error: "Forbidden" }, 403);
      return c.json(membership.state.events);
    });
    app.put("/membership/discounts/:kind/:id", async (c) => {
      const current = identity?.authenticate(
        c.req.header("Authorization")?.replace(/^Bearer /, ""),
      );
      if (
      ...
      try {
        const kind =
          c.req.param("kind") === "products"
            ? "products"
            : c.req.param("kind") === "ticketPackages"
              ? "ticketPackages"
              : null;
        if (!kind) return c.json({ error: "Invalid discount scope" }, 400);
        const body = await c.req.json<{ amount: number }>();
        const result = membership.setDiscount(
          kind,
          c.req.param("id"),
          body.amount,
        );
        if (realtime)
          publishReportEvent(realtime.registry, {
            type: "report-changed",
            source: "membership-discount",
          });
        return c.json(result);
      } catch {
        return c.json({ error: "Discount cannot be configured" }, 400);
      }
    });
    app.get("/membership/discounts", (c) => {
      const current = identity?.authenticate(
        c.req.header("Authorization")?.replace(/^Bearer /, ""),
      );
      if (!current || !identity?.can(current, "read"))
        return c.json({ error: "Unauthorized" }, 401);
      return c.json(membership.state.discounts);
    });
  }
  ...
  if (sales) {
    app.post("/sales", async (c) => {
      ...
      try {
        ...
        if (
          lines.some(
            (line: { outOfStockException?: unknown }) =>
              line.outOfStockException,
          ) &&
          current.user?.role !== "Owner"
        )
          return c.json(
            {
              error: "Owner authorization required for out-of-stock exception",
            },
            403,
          );
        const at = Date.now();
        const result = sales.complete({
          ...
          cashierId: current.user?.id ?? "cashier",
          at,
        });
        if (realtime)
          publishReportEvent(realtime.registry, {
    ...
    app.post("/sales/:id/void", async (c) => {
      ...
      try {
        ...
        return c.json({ error: "Sale cannot be voided" }, 409);
      }
    });
    app.post("/sales/:id/corrections", async (c) => {
      const current = identity?.authenticate(
        c.req.header("Authorization")?.replace(/^Bearer /, ""),
      );
      if (
      ...
      try {
        ...
        const result = sales.addCorrection({
          ...body,
          saleId: c.req.param("id"),
        ...
        return c.json({ error: "Correction cannot be recorded" }, 409);
      }
    });
    app.get("/sales", (c) => {
      const current = identity?.authenticate(
    ...
    app.get("/sales/:id", (c) => {
      ...
      if (!current || !identity?.can(current, "read"))
        return c.json({ error: "Unauthorized" }, 401);
      const sale = sales.get(c.req.param("id"));
      return sale ? c.json(sale) : c.json({ error: "Sale not found" }, 404);
    });
    app.get("/sales/:id/artifacts/:kind", (c) => {
      ...
      if (!current || !identity?.can(current, "read"))
        return c.json({ error: "Unauthorized" }, 401);
      try {
        const artifact = sales.artifact(
          c.req.param("id"),
          c.req.param("kind") as "tickets" | "receipt",
        );
    ...
    app.get("/sales/:id/tickets/:ticketId/qr", async (c) => {
      ...
      try {
        ...
        return c.json({ error: "QR unavailable" }, 404);
      }
    });
    app.post("/sales/:id/refunds", async (c) => {
      ...
      if (!current || !identity?.can(current, "write"))
        return c.json({ error: "Forbidden" }, 403);
      try {
        const sale = sales.get(c.req.param("id"));
        if (!sale || sale.status !== "completed")
          return c.json({ error: "Sale unavailable" }, 409);
        const body = await c.req.json<{
        ...
        const line = sale.lines.find(
          (candidate) =>
            candidate.kind === "product" && candidate.lineId === body.lineId,
        );
        if (!line || line.kind !== "product")
          return c.json({ error: "Product line not found" }, 404);
        const refunded =
          inventory?.refunds
            .filter(
              (item) => item.saleId === sale.id && item.lineId === body.lineId,
            )
            .reduce((sum, item) => sum + item.quantity, 0) ?? 0;
        if (refunded + body.quantity > line.quantity)
          return c.json({ error: "Refund exceeds sold quantity" }, 409);
        const result = inventory!.refund({
          ...body,
          saleId: sale.id,
          productId: line.productId,
          actorId: current.user?.id ?? "cashier",
        });
        if (realtime)
          publishReportEvent(realtime.registry, {
            type: "report-changed",
        ...
        return c.json({ error: "Refund cannot be recorded" }, 409);
      }
    });
    app.post("/sales/:id/print-attempts", async (c) => {
      ...
      try {
        ...
        return c.json({ error: "Print attempt cannot be recorded" }, 409);
      }
    });
  }
  ...
  if (calendar) {
    app.get("/calendar/schedule", (c) => {
      ...
      try {
        return c.json(calendar!.effectiveSchedule(date));
      } catch {
    ...
    app.get("/calendar/packages/:id/snapshot", (c) => {
      ...
      if (!date) return c.json({ error: "date is required" }, 400);
      try {
        return c.json(calendar.snapshot(c.req.param("id"), date));
      } catch {
        return c.json({ error: "Package unavailable or venue closed" }, 409);
      }
    });
  }
  ...
  if (identity) {
    if (realtime) {
      app.get(
        ...
        upgradeWebSocket((c) => {
          ...
          if (!decision.allowed)
            return { onOpen: (_event, ws) => ws.close(1008, decision.reason) };
          return {
            onOpen: (_event, ws) => {
              if (
              ...
              const unregister = realtime.registry.register(decision.deviceId, {
            ...
            onMessage: (event, ws) => {
              if (event.data === "refresh")
                ws.send(
                  JSON.stringify({ type: "synchronized", at: Date.now() }),
                );
              else if (event.data === "ping") ws.send("pong");
              else
                ws.send(
                  JSON.stringify({ type: "error", code: "unknown-command" }),
                );
            },
          };
        }),
    ...
    app.get("/notifications/settings", (c) => {
      ...
      if (!current || !identity.can(current, "read"))
        return c.json({ error: "Unauthorized" }, 401);
      return c.json(
        notifications?.settings.get(current.device.id) ?? {
          soundEnabled: true,
        },
      );
    });
    app.patch("/notifications/settings", async (c) => {
      ...
      return c.json(
        notifications?.configure(
          current.device.id,
          await c.req.json<{ soundEnabled?: boolean }>(),
        ),
      );
    });
    ...
    app.patch("/notifications/routes/:kind", async (c) => {
      ...
      try {
        ...
        if (!Array.isArray(body.modes))
          return c.json({ error: "modes must be an array" }, 400);
        return c.json(
          notifications?.configureRoutes(
            c.req.param("kind") as Parameters<
              NonNullable<NotificationService>["configureRoutes"]
            >[0],
            body.modes,
          ),
        );
      } catch {
        return c.json({ error: "Route configuration is invalid" }, 400);
      }
    });
    ...
    app.post("/pairing/devices/:id/revoke", (c) => {
      ...
      if (
        ...
        return c.json({ error: "Forbidden" }, 403);
      if (c.req.param("id") === identity.ownerDevice()?.id)
        return c.json({ error: "Cannot revoke the owner device" }, 409);
      return identity.revokeDevice(c.req.param("id"))
        ? c.json({ ok: true })
    ...
    app.get("/auth/bootstrap-status", (c) =>
      c.json({
        required: !identity.isBootstrapped(),
        ownerDevice: identity.ownerDevice(),
      }),
    );
    app.post("/auth/bootstrap", async (c) => {
      if (!rateLimit(c, 5, 60_000))
    ...
    app.post("/pairing/invitations", async (c) => {
      ...
      if ((body.kind ?? "private") === "private" && !body.staff)
        return c.json(
          { error: "staff details are required for private devices" },
          400,
        );
      ...
      if (body.staff) {
        ...
        if (role !== "Cashier" && role !== "Staff")
          return c.json({ error: "staff role must be Cashier or Staff" }, 400);
        staff = { name, role };
      }
      return c.json(
        identity.createEnrollment(
          body.origin,
          body.kind ?? "private",
          60_000,
          staff,
    ...
    app.post("/pairing/redeem", async (c) => {
      ...
      if (!body.token || !body.mode)
        return c.json({ error: "token and mode are required" }, 400);
      try {
        return c.json(
          identity.pair(body.token, body.mode, c.req.header("Origin")),
          201,
        );
      } catch (e) {
        ...
        return c.json(
          { error: "Enrollment invitation is invalid or expired" },
          409,
        );
      }
    });
      ...
      try {
        return c.json(
          identity.login(
            device.id,
            "owner",
            (await c.req.json<{ password?: string }>()).password ?? "",
          ),
        );
      } catch {
    ...
    app.post("/auth/login", async (c) => {
      ...
      if (!body.deviceId || !body.username || !body.password)
        return c.json(
          { error: "deviceId, username and password are required" },
          400,
        );
      try {
        return c.json(
          identity.login(body.deviceId, body.username, body.password),
        );
      } catch (e) {
        const msg = e instanceof Error ? e.message : "";
    ...
    app.get("/auth/capability/:capability", (c) => {
      ...
      return c.json({
        allowed: identity.can(current, c.req.param("capability")),
      });
    });
    app.post("/calendar/configure", async (c) => {
      ...
      const body = await c.req.json<{
        timezone?: string;
        day?: import("./calendar.ts").Weekday;
        hours?: import("./calendar.ts").DailyHours;
        override?: import("./calendar.ts").ScheduleOverride;
        package?: Parameters<CalendarStore["upsertPackage"]>[0];
      }>();
      try {
        calendar.configure(body, current.user.id);
        if (realtime)
          publishReportEvent(realtime.registry, {
            type: "report-changed",
            source: "calendar",
        ...
        return c.json({ error: "Invalid calendar configuration" }, 400);
      }
    });
    app.delete("/calendar/overrides/:date", (c) => {
      ...
      if (
        ...
        return c.json({ error: "Forbidden" }, 403);
      return calendar.deleteOverride(c.req.param("date"), current.user.id)
        ? c.json({ ok: true })
        : c.json({ error: "Override not found" }, 404);
    });
    app.delete("/calendar/packages/:id", (c) => {
      ...
      if (
        ...
        return c.json({ error: "Forbidden" }, 403);
      return calendar.deletePackage(c.req.param("id"), current.user.id)
        ? c.json({ ok: true })
