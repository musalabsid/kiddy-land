...
export type TicketPackage = { id: string; name: string; includedMinutes: number | null; weekdayPrice: number; weekendPrice: number; overridePrices: Record<string, number>; overtimeRate: number; overtimeThreshold: number; overtimePercentage: number; deposit: number; depositPolicy: DepositPolicy; active: boolean; version: number };
...
export function createCalendarStore(initial?: { timezone?: string; database?: LocalDatabase }) {
  ...
  if (persisted) {
    ...
    for (const packageValue of JSON.parse(persisted.packages) as TicketPackage[]) { if ((packageValue as unknown as { overtimeThreshold?: number }).overtimeThreshold == null) (packageValue as unknown as { overtimeThreshold: number }).overtimeThreshold = 5; if ((packageValue as unknown as { overtimePercentage?: number }).overtimePercentage == null) (packageValue as unknown as { overtimePercentage: number }).overtimePercentage = 10; packages.set(packageValue.id, packageValue); }
  ...
  function deleteOverride(date: string, actorId: string) { assertDate(date); if (!overrides.has(date)) return false; overrides.delete(date); audit.push({ action: "calendar.override.remove", at: Date.now(), actorId, details: { date } }); persist(); return true; }
  ...
  function canOperate(date: string, at: string, action = "operate") {
    ...
    if ("closed" in schedule.hours) return { allowed: false, reason: schedule.closureReason ?? "Venue is closed", schedule };
    ...
    if (atMin < openMin) return { allowed: false, reason: `Venue is closed outside ${schedule.hours.open}-${schedule.hours.close}`, schedule };
    if (atMin >= closeMin && atMin < graceEnd) {
      if (action === "admit") return { allowed: false, reason: "Venue is in grace period — no entry, ticket expired", schedule };
      return { allowed: false, reason: `Venue is closed outside ${schedule.hours.open}-${schedule.hours.close} (grace: no scanning, inside play continues)`, schedule };
    }
    if (atMin >= graceEnd) return { allowed: false, reason: `Venue is closed outside ${schedule.hours.open}-${schedule.hours.close}`, schedule };
    return { allowed: true, reason: action, schedule };
  }
  function upsertPackage(input: Omit<TicketPackage, "id" | "version" | "active"> & { id?: string }, actorId: string) { if (input.overtimeThreshold != null && (!Number.isInteger(input.overtimeThreshold) || input.overtimeThreshold < 0)) throw new Error("Threshold must be 0 or more"); if (input.overtimePercentage != null && (input.overtimePercentage < 0 || input.overtimePercentage > 100)) throw new Error("Percentage must be 0-100"); if (input.includedMinutes !== null && (!Number.isInteger(input.includedMinutes) || input.includedMinutes <= 0)) throw new Error("Included duration must be positive"); for (const date of Object.keys(input.overridePrices)) assertDate(date); for (const amount of [input.weekdayPrice, input.weekendPrice, input.overtimeRate, input.deposit, ...Object.values(input.overridePrices)]) if (!Number.isInteger(amount) || amount < 0) throw new Error("Amounts must be non-negative IDR integers"); // ponytail: gradual now allowed for any package
  const previous = input.id ? packages.get(input.id) : undefined; const value: TicketPackage = { ...input, id: previous?.id ?? input.id ?? id(), version: (previous?.version ?? 0) + 1, active: true }; packages.set(value.id, value); audit.push({ action: "ticket-package.upsert", at: Date.now(), actorId, details: { id: value.id, version: value.version } }); persist(); return value; }
  ...
  return { get timezone() { return timezone; }, weekly, overrides, packages, audit, setTimezone, setWeeklyHours, setOverride, deleteOverride, effectiveSchedule, operatingDate, operatingTime, canOperate, upsertPackage, deletePackage, configure, snapshot };
}
