...
export type PackageSnapshot = Omit<TicketPackage, "overridePrices"> & { overridePrice?: number; price: number; pricePeriod: PricePeriod; operatingDate: string };
...
export function createCalendarStore(initial?: { timezone?: string }) {
  ...
  function snapshot(packageId: string, date: string): PackageSnapshot { const packageValue = packages.get(packageId); if (!packageValue || !packageValue.active) throw new Error("Ticket Package is unavailable"); const schedule = effectiveSchedule(date); if ("closed" in schedule.hours) throw new Error(schedule.closureReason ?? "Venue is closed"); const overridePrice = packageValue.overridePrices[date]; const price = overridePrice ?? (schedule.period === "weekend" ? packageValue.weekendPrice : packageValue.weekdayPrice); return { ...packageValue, overridePrice, price, pricePeriod: schedule.period, operatingDate: date }; }
