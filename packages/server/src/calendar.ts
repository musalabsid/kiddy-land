...
function assertDate(date: string) { if (!/^\d{4}-\d{2}-\d{2}$/.test(date) || Number.isNaN(new Date(`${date}T00:00:00Z`).getTime())) throw new Error("Date must use YYYY-MM-DD"); }
...
export function createCalendarStore(initial?: { timezone?: string; database?: LocalDatabase }) {
  ...
  function upsertPackage(input: Omit<TicketPackage, "id" | "version" | "active"> & { id?: string }, actorId: string) { if (input.includedMinutes !== null && (!Number.isInteger(input.includedMinutes) || input.includedMinutes <= 0)) throw new Error("Included duration must be positive"); for (const amount of [input.weekdayPrice, input.weekendPrice, input.overtimeRate, input.deposit, ...Object.values(input.overridePrices)]) if (!Number.isInteger(amount) || amount < 0) throw new Error("Amounts must be non-negative IDR integers"); if (input.depositPolicy === "unlimited-cap" && input.includedMinutes !== null) throw new Error("Unlimited-cap policy requires an Unlimited Package"); const previous = input.id ? packages.get(input.id) : undefined; const value: TicketPackage = { ...input, id: previous?.id ?? input.id ?? id(), version: (previous?.version ?? 0) + 1, active: true }; packages.set(value.id, value); audit.push({ action: "ticket-package.upsert", at: Date.now(), actorId, details: { id: value.id, version: value.version } }); persist(); return value; }
  ...
  return { get timezone() { return timezone; }, weekly, overrides, packages, audit, setTimezone, setWeeklyHours, setOverride, effectiveSchedule, operatingDate, operatingTime, canOperate, upsertPackage, snapshot };
}
