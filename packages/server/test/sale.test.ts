...
describe("cashier ticket sale", () => {
  ...
  test("atomically creates one receipt and independent tickets", () => {
    ...
    const sale = store.complete({
      idempotencyKey: "checkout-1",
      cashierId: "cashier",
      operatingDate: "2024-01-01",
      paymentMethod: "cash",
      lines: [
        { childId: "child-1", packageId: pkg.id },
        { childId: "child-2", packageId: pkg.id },
      ],
    });
  ...
  test("allows at most 12 tickets per sale", () => {
    ...
    expect(() =>
      store.complete({
        idempotencyKey: "thirteen",
        cashierId: "cashier",
        operatingDate: "2024-01-01",
        paymentMethod: "cash",
        lines: [...lines, { childId: "child-13", packageId: pkg.id }],
      }),
    ).toThrow("12 tickets");
  });
  test("duplicate submit returns original sale", () => {
    ...
    const input = {
      ...
      lines: [{ childId: "child", packageId: pkg.id, paymentConfirmed: true }],
    };
  ...
  test("limits each product line to 24 units", () => {
    ...
    const product = inventory.create(
      { sku: "LIMIT", name: "Limited product", price: 100, stock: 30 },
      "owner",
    );
    const sales = createSaleStore(calendar, undefined, inventory);
    expect(
      sales.complete({
        idempotencyKey: "product-24",
        cashierId: "cashier",
        operatingDate: "2024-01-01",
        paymentMethod: "cash",
        lines: [{ kind: "product", productId: product.id, quantity: 24 }],
      }).lines[0]?.kind,
    ).toBe("product");
    expect(() =>
      sales.complete({
        idempotencyKey: "product-25",
        cashierId: "cashier",
        operatingDate: "2024-01-01",
        paymentMethod: "cash",
        lines: [{ kind: "product", productId: product.id, quantity: 25 }],
      }),
    ).toThrow("1 to 24");
  });
  ...
  test("does not partially reserve duplicate product lines", () => {
    ...
    const inventory = createInventoryStore();
    const product = inventory.create(
      { sku: "P", name: "Product", price: 100, stock: 1 },
      "owner",
    );
    const sales = createSaleStore(calendar, undefined, inventory);
    expect(() =>
      sales.complete({
        ...
        paymentMethod: "cash",
        lines: [
          { kind: "product", productId: product.id, quantity: 1 },
          { kind: "product", productId: product.id, quantity: 1 },
        ],
      }),
  ...
  test("rejects sales outside the venue operating window", () => {
    ...
    expect(() =>
      store.complete({
        ...
        cashierId: "cashier",
        operatingDate: "2024-01-01",
        at: Date.parse("2024-01-01T15:00:00Z"),
        paymentMethod: "cash",
  ...
  test("rejects invalid or partial payment methods", () => {
    ...
    expect(() =>
      store.complete({
        ...
        operatingDate: "2024-01-01",
        paymentMethod: "cash",
        lines: [{ childId: "child", packageId: "missing" }],
      }),
    ).toThrow();
    expect(() =>
      store.complete({
        idempotencyKey: "bad-payment",
        cashierId: "cashier",
        operatingDate: "2024-01-01",
        paymentMethod: "cash+QRIS" as never,
        lines: [{ childId: "child", packageId: pkg.id }],
      }),
    ).toThrow();
  });
