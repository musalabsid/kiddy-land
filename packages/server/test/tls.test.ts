...
describe("local HTTPS listener", () => {
  test("generates a persisted self-signed cert and serves the same app over HTTPS", async () => {
    ...
    const runtime = createHostRuntime({ dataDir, host: "127.0.0.1", port: 43137, httpsPort: 43138, tlsHosts: ["192.168.1.108"] });
    await runtime.start();
    try {
      expect(runtime.server.httpsUrl).toBe("https://127.0.0.1:43138");
      expect(runtime.server.tlsFingerprint).toMatch(/^([0-9A-F]{2}:){31}[0-9A-F]{2}$/);
      ...
      const httpReady = await (await fetch(runtime.server.url + "/ready")).json() as { status: string };
