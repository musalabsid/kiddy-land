...
function calendarForValidation() {
...
describe("venue calendar and ticket packages", () => {
  test("derives venue-local operating dates and schedule overrides", () => {
    ...
    expect(calendar.operatingDate(new Date("2024-01-01T17:30:00Z"))).toBe(
      "2024-01-02",
    );
    ...
    expect(calendar.effectiveSchedule("2024-01-01").hours).toEqual({
      open: "09:00",
      close: "18:00",
    });
    calendar.setOverride(
      { date: "2024-01-01", kind: "closed", reason: "Public holiday" },
      "owner",
    );
    expect(calendar.canOperate("2024-01-01", "12:00").reason).toBe(
      "Public holiday",
    );
  });
  ...
  test("supports pricing overrides and immutable package snapshots", () => {
    ...
    const packageValue = calendar.upsertPackage(
      {
        name: "90 Minutes",
        includedMinutes: 90,
        weekdayPrice: 50000,
    ...
    const changed = calendar.upsertPackage(
      { ...packageValue, weekdayPrice: 60000 },
      "owner",
    );
  ...
  test("persists venue configuration and audit history across store instances", async () => {
    ...
    const packageValue = first.upsertPackage(
      {
        name: "Persisted",
        includedMinutes: 60,
        weekdayPrice: 40000,
  ...
  test("validates unlimited packages and operating boundaries", () => {
    expect(() =>
      calendarForValidation().upsertPackage(
        {
          name: "Bad",
          includedMinutes: 60,
          weekdayPrice: 1,
          weekendPrice: 1,
          overridePrices: { "2024-01-01": -1 },
          overtimeRate: 0,
          deposit: 0,
          depositPolicy: "return-remainder",
        },
