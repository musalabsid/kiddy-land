...
describe("identity persistence", () => {
  test("keeps devices and revocations across identity store recreation", async () => {
    ...
    const first = createIdentityStore({ ownerPassword: "secret", database: firstDb });
    ...
    const activeDevice = first.pair(activeInvitation.token, "Owner Dashboard").device;
    ...
    expect(() => second.login(activeDevice.id, "owner", "secret")).not.toThrow();
    secondDb.close();
