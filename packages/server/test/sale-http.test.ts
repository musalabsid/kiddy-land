...
describe("cashier sale HTTP workflow", () => {
  test("completes one sale, exposes artifacts, and records a failed print without changing it", async () => {
    ...
    const runtime = createHostRuntime({ dataDir, port: 43133 });
    ...
    for (const day of [
      ...
      await fetch(`${base}/calendar/configure`, {
        method: "POST",
        headers: ownerAuth,
        body: JSON.stringify({ day, hours: { open: "00:00", close: "23:59" } }),
      });
    await fetch(`${base}/calendar/configure`, {
      method: "POST",
      headers: ownerAuth,
    ...
    const config = (await (
      await fetch(`${base}/calendar/config`, { headers: ownerAuth })
    ).json()) as { packages: Array<{ id: string }> };
    const cashierInvite = (await (
    ...
    const cashierPair = (await (
      await fetch(`${base}/pairing/redeem`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ token: cashierInvite.token, mode: "Cashier" }),
      })
    ).json()) as { device: { id: string } };
    const cashierLogin = (await (
      await fetch(`${base}/auth/login`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          deviceId: cashierPair.device.id,
          username: "owner",
          password: "change-me",
        }),
      })
    ...
    const pdf = await fetch(`${base}/sales/${created.id}/artifacts/tickets`, {
      headers: { authorization: `Bearer ${cashierLogin.token}` },
    });
    expect(pdf.status).toBe(200);
    expect(pdf.headers.get("content-type")).toContain("application/pdf");
    const print = await fetch(`${base}/sales/${created.id}/print-attempts`, {
      method: "POST",
      headers: auth,
      body: JSON.stringify({
        artifact: "tickets",
    ...
    const unchanged = (await (
      await fetch(`${base}/sales/${created.id}`, {
        headers: { authorization: `Bearer ${cashierLogin.token}` },
      })
