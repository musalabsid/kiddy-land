...
describe("ticket and play session lifecycle", () => {
  ...
  test("admits once and duplicate entry is state-aware", () => {
    ...
    const entered = lifecycle.admit(ticket.code, Date.parse("2024-01-01T10:00:00Z"));
    expect(entered.ok).toBe(true);
    expect(lifecycle.admit(ticket.code).message).toBe("Ticket already admitted");
  ...
  test("finite session settles overtime and deposit remainder", () => {
    ...
    const result = lifecycle.exit(ticket.code, Date.parse("2024-01-01T11:30:00Z"));
  ...
  test("unlimited package has no overtime during play", () => {
    ...
    lifecycle.admit(ticket.code, Date.parse("2024-01-01T10:00:00Z"));
    const result = lifecycle.exit(ticket.code, Date.parse("2024-01-01T19:00:00Z"));
  ...
  test("recovery keeps ticket identity and closing expires waiting tickets", () => {
    ...
    const closed = second.lifecycle.close("2024-01-01", Date.parse("2024-01-01T20:00:00Z"));
