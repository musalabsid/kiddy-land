...
describe("ticket lifecycle HTTP contract", () => {
  test("scanner routes perform idempotent entry/exit and recovery without changing ticket identity", async () => {
    ...
    const exited = await json(app, "/tickets/scan/exit", { method: "POST", headers: { authorization: `Bearer ${exit.token}` }, body: JSON.stringify({ code: sale.tickets[0]!.code }) });
    expect(exited.body.state).toBe("completed"); expect(exited.body.session.outstandingCharge).toBe(0);
    const duplicateExit = await json(app, "/tickets/scan/exit", { method: "POST", headers: { authorization: `Bearer ${exit.token}` }, body: JSON.stringify({ code: sale.tickets[0]!.code }) });
    expect(duplicateExit.body.message).toBe("Ticket already settled");

    const collectAttempt = await json(app, "/tickets/" + sale.tickets[0]!.id + "/collect-charge", { method: "POST", headers: { authorization: `Bearer ${exit.token}` }, body: JSON.stringify({ amount: 1, paymentMethod: "cash" }) });
