...
describe("restricted public kiosk", () => {
  test("validates tickets and exposes only public product fields", async () => {
    ...
    const ticket = await json(app, "/public/tickets/validate", {
      method: "POST",
      headers: auth,
    ...
    const products = await json(app, "/public/products", { headers: auth });
    ...
    expect(
      (await json(app, "/products", { headers: auth })).response.status,
    ).toBe(401);
    expect(
      (await json(app, "/sales", { method: "POST", headers: auth, body: "{}" }))
        .response.status,
    ).toBe(403);
  });
});
