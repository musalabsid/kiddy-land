...
describe("restricted public kiosk", () => {
  test("validates tickets and exposes only public product fields", async () => {
    ...
    const invite = await json(app, "/pairing/invitations", { method: "POST", body: JSON.stringify({ origin: "http://local", kind: "public-kiosk" }) }); const paired = await json(app, "/pairing/redeem", { method: "POST", headers: { origin: "http://local" }, body: JSON.stringify({ token: invite.body.token, mode: "Public Kiosk" }) }); const auth = { authorization: `Bearer ${paired.body.session.token}` };
