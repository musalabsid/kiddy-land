...
describe("staff-invite pairing", () => {
  test("private invitations require staff details", async () => {
    ...
    const response = await json(app, "/pairing/invitations", {
      method: "POST",
      headers: {
  ...
  test("invite with staff creates a staff user and auto-logs-in the device", async () => {
    ...
    const invite = await json(app, "/pairing/invitations", {
      method: "POST",
    ...
    const pair = await json(app, "/pairing/redeem", {
      method: "POST",
      headers: { origin: "http://local" },
      body: JSON.stringify({ token: invite.body.token, mode: "Cashier" }),
    });
    expect(pair.status).toBe(201);
    expect(pair.body.session?.token).toBeTruthy();
    // the session is bound to the created staff user
    const session = await json(app, "/auth/session", {
      headers: { authorization: `Bearer ${pair.body.session.token}` },
    });
    ...
    const again = await json(app, "/pairing/redeem", {
      method: "POST",
      headers: { origin: "http://local" },
      body: JSON.stringify({ token: invite.body.token, mode: "Cashier" }),
    });
  ...
  test("kiosk invite without staff still auto-logs-in (existing behavior)", async () => {
    ...
    const owner = await appBootstrapOwner(app);
    const kiosk = await appPairDevice(
      app,
      owner.token,
      "Public Kiosk",
      "public-kiosk",
...
describe("device deletion", () => {
  test("delete removes the device permanently (hard delete)", async () => {
    ...
    const list = await json(app, "/pairing/devices", {
      headers: { authorization: `Bearer ${owner.token}` },
    });
    expect(
      list.body.devices.some((d: { id: string }) => d.id === paired.deviceId),
    ).toBe(true);
    const del = await json(app, "/pairing/devices/" + paired.deviceId, {
      method: "DELETE",
      headers: { authorization: `Bearer ${owner.token}` },
    });
    expect(del.status).toBe(200);
    const after = await json(app, "/pairing/devices", {
      headers: { authorization: `Bearer ${owner.token}` },
    });
    expect(
      after.body.devices.some((d: { id: string }) => d.id === paired.deviceId),
    ).toBe(false);
    // non-owner cannot delete
    const guest = await json(app, "/pairing/devices/" + paired.deviceId, {
      method: "DELETE",
...
describe("owner device protection", () => {
  test("cannot revoke or delete the owner device", async () => {
    ...
    const revoke = await json(app, `/pairing/devices/${ownerDeviceId}/revoke`, {
      method: "POST",
      headers: { authorization: `Bearer ${owner.token}` },
      body: "{}",
    });
    expect(revoke.status).toBe(409);
    const del = await json(app, `/pairing/devices/${ownerDeviceId}`, {
      method: "DELETE",
      headers: { authorization: `Bearer ${owner.token}` },
    });
    ...
    expect(
      list.body.devices.some((d: { id: string }) => d.id === ownerDeviceId),
    ).toBe(true);
  });
...
describe("device self-logout deletion", () => {
  test("a non-owner device can delete itself, but not another device", async () => {
    ...
    const denied = await json(app, `/pairing/devices/${other.deviceId}`, {
      method: "DELETE",
      headers: { authorization: `Bearer ${staff.token}` },
    });
    expect(denied.status).toBe(403);
    const self = await json(app, `/pairing/devices/${staff.deviceId}`, {
      method: "DELETE",
      headers: { authorization: `Bearer ${staff.token}` },
    });
